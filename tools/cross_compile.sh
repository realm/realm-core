...
if [ "${OS}" == "android" ]; then
    ...
    if [ "$SDK" = "iphone" ]; then
      cp "src/realm/${BUILD_TYPE}-${SDK}os/librealm${suffix}.a" \
         "src/realm/${BUILD_TYPE}/librealm${suffix}.a"
      cp "src/realm/parser/${BUILD_TYPE}-${SDK}os/librealm-parser${suffix}.a" \
         "src/realm/parser/${BUILD_TYPE}/librealm-parser${suffix}.a"
    else
      lipo -create \
