...
* Changeset upload batching did not calculate the accumulated size correctly, resulting in "error reading body failed to read: read limited at 16777217 bytes" errors from the server when writing large amounts of data (since 11.13.0).([#5373]https://github.com/realm/realm-core/issues/5373)

### Breaking changes
* For synchronized Realms, the history version has been bumped, the old history is upgraded automatically, 
  but multiprocess access requires that all clients has been updated to this version.
