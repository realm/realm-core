...
void AdaptiveStringColumn::LeafFindAll(Column &result, const char* value) const {
	if (IsLongStrings()) {
		return ((ArrayStringLong*)m_array)->FindAll(result, value);
	}
	else {
		return ((ArrayString*)m_array)->FindAll(result, value);
	}
}
