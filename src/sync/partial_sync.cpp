...
void initialize_schema(Group& group)
{
    ...
    // autogenerated.
    if (!table->get_column_key(property_name)) {
        auto idx = table->add_column(type_String, property_name);
    ...
    // is created.
    if (!table->get_column_key(property_created_at)) {
    ...
    if (!table->get_column_key(property_updated_at)) {
    ...
    if (!table->get_column_key(property_time_to_live)) {
    ...
    if (!table->get_column_key(property_expires_at)) {
