...
size_t GroupWriter::write_group()
{
    ...
    TIGHTDB_ASSERT_NEW(fpositions.size(), ==, flengths.size());
    ...
    TIGHTDB_ASSERT_NEW(reserve_size, >, max_free_space_needed);
    ...
    TIGHTDB_ASSERT_NEW(end_pos, <=, reserve_pos + max_free_space_needed);
...
{
    TIGHTDB_ASSERT_NEW(size % 8, ==, 0); // 8-byte alignment
    ...
    TIGHTDB_ASSERT_NEW(chunk_size, >=, size);
...
pair<size_t, size_t> GroupWriter::extend_free_space(size_t requested_size)
{
    ...
    if (!positions.is_empty()) {
        ...
        if (!in_use) {
            ...
            TIGHTDB_ASSERT_NEW(last_size, <, extend_size);
            TIGHTDB_ASSERT_NEW(last_pos + last_size, <=, logical_file_size);
    ...
    TIGHTDB_ASSERT_NEW(stop_doubling_size % 8, ==, 0);
    ...
    while (new_file_size < min_file_size) {
        if (new_file_size < stop_doubling_size) {
            ...
            TIGHTDB_ASSERT_NEW(new_file_size, !=, 0);
            // Be sure that the doubling does not overflow
            TIGHTDB_ASSERT_NEW(stop_doubling_size, <=, numeric_limits<size_t>::max() / 2);
            new_file_size *= 2;
        }
        else {
    ...
    TIGHTDB_ASSERT_NEW(new_file_size % 8, ==, 0);
    ...
    if (extend_last_chunk) {
        ...
        TIGHTDB_ASSERT_NEW(chunk_size % 8, ==, 0); // 8-byte alignment
...
void GroupWriter::write(const char* data, size_t size)
{
    ...
    TIGHTDB_ASSERT_NEW((pos & 0x7), ==, 0); // Write position should always be 64bit aligned
...
{
    TIGHTDB_ASSERT_NEW(pos + size, <=, to_size_t(m_group.m_top.get(2) / 2));
    TIGHTDB_ASSERT_NEW(pos + size, <=, m_file_map.get_size());
    char* dest_addr = m_file_map.get_addr() + pos;
