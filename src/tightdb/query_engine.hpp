...
public:
    ...
    template <int action>bool match_callback(int64_t v) {
        if (v >= m_leaf_end_agg) {
            m_column_agg->GetBlock(v, m_array_agg, m_leaf_start_agg);
            const size_t leaf_size = m_array_agg.Size();
            m_leaf_end_agg = m_leaf_start_agg + leaf_size;
        }

        int64_t av = m_array_agg.Get(v - m_leaf_start_agg);
        bool b = m_array.FIND_ACTION<action>(v, av, &state, &dummy);
        ...
        if(m_child == 0 && (SameType<F, EQUAL>::value || SameType<F, NOTEQUAL>::value || SameType<F, LESS>::value || SameType<F, GREATER>::value || SameType<F, NONE>::value)) {
            ...
            for (size_t s = start; s < end; ) {
                ...
                if(agg_col == m_column_id || agg_col == -1)
                    f = m_array.find(c, action, m_value, s - m_leaf_start, m_local_end, m_leaf_start, &state);
                else
                    f = m_array.find<F, TDB_CALLBACK_IDX>(m_value, s - m_leaf_start, m_local_end, m_leaf_start, &state, std::bind1st(std::mem_fun(&NODE::match_callback<action>), this));
            ...
            for(;;) {
                ...
                if(agg_col == m_column_id || agg_col == -1)
                    m_array.FIND_ACTION<action>(r, m_column->Get(r), &state, &dummy);
                else
