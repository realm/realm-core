...
// Lets you access elements of an integer column in increasing order in a fast way where leafs are cached. Set
// rand_access = true to make it possible to access in any index pattern, not just increasing order
...
public:
    ...
    TIGHTDB_FORCEINLINE bool cache_next(size_t index)
    {
        ...
        if (index >= m_leaf_end || (true && index < m_leaf_start)) {
...
.equal(3, 660).expression(e).find_first();
...
class ExpressionNode: public ParentNode {
    ...
    size_t find_first_local(size_t start, size_t end)
    {
        size_t res = compare->Compare(start, end);
        return res;


    }
    
    CCompare<Equal>* compare;
};
