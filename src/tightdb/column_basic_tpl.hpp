...
{
    return ColumnBase::aggregate<T, SumType, act_Sum, None>(0, start, end, 0, limit);
}
...
double BasicColumn<T>::average(std::size_t start, std::size_t end, size_t limit) const
{
    ...
    double sum1 = ColumnBase::aggregate<T, SumType, act_Sum, None>(0, start, end, 0, limit);
...
{
    return ColumnBase::aggregate<T, T, act_Min, None>(0, start, end, 0, limit);
}

template<class T>
T BasicColumn<T>::maximum(std::size_t start, std::size_t end, size_t limit) const
{
    return ColumnBase::aggregate<T, T, act_Max, None>(0, start, end, 0, limit);
}
