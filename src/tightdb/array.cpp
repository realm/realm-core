...
void Array::move(size_t begin, size_t end, size_t dest_begin)
{
    ...
    TIGHTDB_ASSERT_NEW(dest_begin, <=, m_size);
    TIGHTDB_ASSERT_NEW(end - begin, <=, m_size - dest_begin);
...
void Array::move_backward(size_t begin, size_t end, size_t dest_end)
{
    TIGHTDB_ASSERT_NEW(begin, <=, end);
    TIGHTDB_ASSERT_NEW(end, <=, m_size);
    TIGHTDB_ASSERT_NEW(dest_end, <=, m_size);
    TIGHTDB_ASSERT_NEW(end - begin, <=, dest_end);
...
{
    TIGHTDB_ASSERT(is_attached());
    TIGHTDB_ASSERT_NEW(size, <=, m_size);
...
void Array::ensure_minimum_width(int64_t value)
{
    ...
    size_t width = bit_width(value);
    TIGHTDB_ASSERT_NEW(width, >, m_width);
...
size_t Array::calc_aligned_byte_size(size_t size, int width)
{
    ...
    TIGHTDB_ASSERT_NEW(byte_size, >, 0);
...
{
    TIGHTDB_ASSERT_NEW(get_wtype_from_header(get_header_from_data(m_data)), ==, wtype_Bits);
...
MemRef Array::clone(const char* header, Allocator& alloc, Allocator& target_alloc)
{
    ...
    TIGHTDB_ASSERT_NEW(get_wtype_from_header(header), ==, wtype_Bits);
...
void Array::alloc(size_t size, size_t width)
{
    ...
    TIGHTDB_ASSERT_NEW(m_capacity, >, 0);
                                    ...
                                    ref_type new_sibling_ref, TreeInsertBase& state)
{
    ...
    if (insert_ndx - 1 >= TIGHTDB_MAX_BPNODE_SIZE) {
        TIGHTDB_ASSERT_NEW(insert_ndx - 1, ==, TIGHTDB_MAX_BPNODE_SIZE);
        ...
        TIGHTDB_ASSERT_NEW(size(), >=, 2);
        size_t num_children = size() - 2;
        TIGHTDB_ASSERT_NEW(num_children, >=, 1); // invar:bptree-nonempty-inner
        // Move some refs over
...
void Array::Verify() const
{
    ...
    TIGHTDB_ASSERT_NEW(ref_in_parent, ==, m_ref);
}
...
VerifyBptreeResult verify_bptree(const Array& node, Array::LeafVerifier leaf_verifier)
{
    ...
    TIGHTDB_ASSERT_NEW(node.get_type(), ==, Array::type_InnerBptreeNode);

    TIGHTDB_ASSERT_NEW(node.size(), >=, 2);
    size_t num_children = node.size() - 2;

    // Verify invar:bptree-nonempty-inner
    TIGHTDB_ASSERT_NEW(num_children, >=, 1);
    ...
    bool general_form;
    {
        ...
        if (general_form) {
            ...
            offsets.Verify();
            TIGHTDB_ASSERT_NEW(offsets.get_type(), ==, Array::type_Normal);
            TIGHTDB_ASSERT_NEW(offsets.size(), ==, num_children - 1);
        }
        else {
    ...
    for (size_t i = 0; i < num_children; ++i) {
        ...
        if (child_is_leaf) {
            ...
            TIGHTDB_ASSERT_NEW(elems_in_child, >= , 1);
        ...
        TIGHTDB_ASSERT_NEW(leaf_level_of_child, ==, leaf_level_of_children);
    ...
    TIGHTDB_ASSERT_NEW(leaf_level_of_children, !=, -1);
    {
        int_fast64_t last_value = node.back();
        TIGHTDB_ASSERT_NEW(last_value % 2, !=, 0);
        size_t total_elems = 0;
        TIGHTDB_ASSERT(!int_cast_with_overflow_detect(last_value/2, total_elems));
        TIGHTDB_ASSERT_NEW(num_elems, ==, total_elems);
    }
...
top:
    ...
    for (;;) {
        ...
        if (!sub_isindex) {
            ...
            TIGHTDB_ASSERT_NEW(method, !=, index_FindAll_nocopy);
    ...
    TIGHTDB_NOEXCEPT_IF(noexcept(handler(Array::NodeInfo())))
{
    ...
    size_t elems_per_child = 0;
    {
        TIGHTDB_ASSERT_NEW(node.size(), >=, 1);
        int_fast64_t first_value = node.get(0);
                                  ...
                                  Array::EraseHandler& handler)
{
    ...
    if (child_is_leaf) {
        ...
        TIGHTDB_ASSERT_NEW(child_size, >=, 2);
        size_t num_grandchildren = child_size - 2;
        TIGHTDB_ASSERT_NEW(num_grandchildren, >=, 1); // invar:bptree-nonempty-inner
...
void Array::erase_bptree_elem(Array* root, size_t elem_ndx, EraseHandler& handler)
{
    ...
    TIGHTDB_ASSERT_NEW(root->size(), >=, 1 + 1 + 1); // invar:bptree-nonempty-inner
    ...
    if (destroy_root) {
        ...
        TIGHTDB_ASSERT_NEW(root->size(), >=, 2);
...
bool Array::do_erase_bptree_elem(size_t elem_ndx, EraseHandler& handler)
{
    ...
    if (destroy_child) {
        ...
        TIGHTDB_ASSERT_NEW(num_children, >=, 2);
