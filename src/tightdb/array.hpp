...
{
    TIGHTDB_ASSERT_NEW(value, <=, 0xFFFFFFL);
    typedef unsigned char uchar;
    uchar* h = reinterpret_cast<uchar*>(header);
...
template<class S> std::size_t Array::write(S& out, bool recurse, bool persist) const
{
    ...
    if (!recurse || !m_has_refs) {
        ...
        TIGHTDB_ASSERT_NEW(array_pos % 8, ==, 0); // 8-byte alignment
    ...
    try {
        ...
        for (std::size_t i = 0; i != n; ++i) {
            ...
            if (value == 0 || value % 2 != 0) {
                ...
                TIGHTDB_ASSERT_NEW(sub_pos % 8, ==, 0); // 8-byte alignment
...
{
    TIGHTDB_ASSERT_NEW(size(), >=, 1 + 1 + 1); // At least one child
    ...
    if (elem_ndx == 0) {
        ...
        TIGHTDB_ASSERT_NEW(child_ndx, <, size() - 2);
    ...
    bool child_is_leaf = !get_is_inner_bptree_node_from_header(child_header);
    if (child_is_leaf) {
        TIGHTDB_ASSERT_NEW(elem_ndx_in_child, <=, TIGHTDB_MAX_BPNODE_SIZE);
        new_sibling_ref =
            TreeTraits::leaf_insert(MemRef(child_header, child_ref), childs_parent,
                                    child_ref_ndx, m_alloc, elem_ndx_in_child, state); // Throws
    }
...
template<bool eq, size_t width>size_t Array::FindZero(uint64_t v) const
{
    ...
    while (eq == (((v >> (width * start)) & mask) != 0)) {
        ...
        TIGHTDB_ASSERT_NEW(start, <=, 8 * sizeof(v));
        start++;
    }
