...
inline int64_t ColumnMixed::get_int(std::size_t ndx) const TIGHTDB_NOEXCEPT
{
    ...
    if (col_type == mixcol_IntNeg) {
        ...
        TIGHTDB_ASSERT_NEW(col_type, ==, mixcol_Int);
    }
    return value;
}
...
{
    TIGHTDB_ASSERT_NEW(m_types->get(ndx), ==, mixcol_Bool);
...
{
    TIGHTDB_ASSERT_NEW(m_types->get(ndx), ==, mixcol_Date);
...
inline float ColumnMixed::get_float(std::size_t ndx) const TIGHTDB_NOEXCEPT
{
    ...
    TIGHTDB_ASSERT_NEW(m_types->get(ndx), ==, mixcol_Float);
...
inline double ColumnMixed::get_double(std::size_t ndx) const TIGHTDB_NOEXCEPT
{
    ...
    if (col_type == mixcol_DoubleNeg)
        ...
        TIGHTDB_ASSERT_NEW(col_type, ==, mixcol_Double);
    }
...
{
    TIGHTDB_ASSERT_NEW(ndx, <, m_types->size());
    TIGHTDB_ASSERT_NEW(m_types->get(ndx), ==, mixcol_String);
...
{
    TIGHTDB_ASSERT_NEW(ndx, <, m_types->size());
    TIGHTDB_ASSERT_NEW(m_types->get(ndx), ==, mixcol_Binary);
    TIGHTDB_ASSERT(m_binary_data);
...
inline void ColumnMixed::refresh_accessor_tree(std::size_t col_ndx, const Spec& spec)
{
    ...
    if (m_binary_data) {
        TIGHTDB_ASSERT_NEW(m_array->size(), ==, 3);
