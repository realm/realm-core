                 ...
                 bool skip_create_column_accessors)
{
    ...
    TIGHTDB_ASSERT_NEW(m_top.size(), ==, 2);
...
void Table::do_erase_column(Descriptor& desc, size_t col_ndx)
{
    ...
    Table& root_table = df::get_root_table(desc);
    TIGHTDB_ASSERT(!root_table.has_shared_type());
    TIGHTDB_ASSERT_NEW(col_ndx, <, desc.get_column_count());
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <=, m_spec.get_public_column_count());
...
{
    size_t backlink_col_ndx = m_spec.find_backlink_column(origin_table_ndx, origin_col_ndx);
    TIGHTDB_ASSERT_NEW(backlink_col_ndx, !=, tightdb::not_found);
                             ...
                             SubtableUpdater* updater)
{
    ...
    TIGHTDB_ASSERT_NEW(col_path_size, >=, 1);

    size_t col_ndx = *col_path_begin;
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, col_type_Table);
...
void Table::create_degen_subtab_columns()
{
    ...
    for (size_t i = 0; i < num_cols; ++i) {
        ...
        // root table.
        TIGHTDB_ASSERT_NEW(m_spec.get_column_attr(i), ==, col_attr_None);
    }
...
Table::~Table() TIGHTDB_NOEXCEPT
{
    ...
    if (!is_attached()) {
        ...
        TIGHTDB_ASSERT_NEW(m_ref_count, ==, 0);
        return;
    }
...
void Table::validate_column_type(const ColumnBase& column, ColumnType col_type, size_t ndx) const
{
    ...
    if (col_type == col_type_Int) {
        ...
        TIGHTDB_ASSERT_NEW(col_type, ==, real_col_type);
    }
                                ...
                                Allocator& alloc) TIGHTDB_NOEXCEPT
{
    ...
    TIGHTDB_ASSERT_NEW(Array::get_size_from_header(columns_header), !=, 0);
...
{
    TIGHTDB_ASSERT(is_attached());
    TIGHTDB_ASSERT_NEW(row_ndx, <, m_size);
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <, get_column_count());
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, col_type_Table);
    TIGHTDB_ASSERT_NEW(row_ndx, <=, m_size);
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <, get_column_count());
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, col_type_Mixed);
    TIGHTDB_ASSERT_NEW(row_ndx, <=, m_size);
...
Table* Table::get_link_target_table_accessor(size_t col_ndx) TIGHTDB_NOEXCEPT
{
    ...
    TIGHTDB_ASSERT(m_columns.is_attached());
    TIGHTDB_ASSERT_NEW(col_ndx, <, m_cols.size());
    if (ColumnBase* col = m_cols[col_ndx]) {
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <, get_column_count());
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, col_type_Int);
    bump_version();
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <, get_column_count());
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, col_type_Bool);
    TIGHTDB_ASSERT_NEW(ndx, <, m_size);
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <, get_column_count());
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, col_type_DateTime);
    TIGHTDB_ASSERT_NEW(ndx, <, m_size);
...
{
    TIGHTDB_ASSERT_NEW(col_ndx, <, m_columns.size());
    TIGHTDB_ASSERT_NEW(ndx, <, m_size);
    ...
    TIGHTDB_ASSERT_NEW(type, ==, col_type_StringEnum);
...
{
    TIGHTDB_ASSERT_NEW(row_ndx, ==, m_size); // can only append to unorded tables
...
template<class T> size_t Table::find_first(size_t col_ndx, T value) const
{
    ...
    TIGHTDB_ASSERT_NEW(get_real_column_type(col_ndx), ==, ColumnTypeTraits3<T>::ct_id_real);
                      ...
                      const Column* viewrefs) const
{
    ...
    TIGHTDB_ASSERT_NEW(group_by_column, <, m_columns.size());
    TIGHTDB_ASSERT_NEW(aggr_column, <, m_columns.size());

    TIGHTDB_ASSERT_NEW(get_column_type(group_by_column), ==, type_String);
...
bool Table::compare_rows(const Table& t) const
{
    ...
    TIGHTDB_ASSERT_NEW(t.get_column_count(), ==, n);
    for (size_t i = 0; i != n; ++i) {
        ColumnType type = get_real_column_type(i);
        ...
        switch (type) {
            ...
            case col_type_String: {
                ...
                if (type2 == col_type_String) {
                    ...
                    TIGHTDB_ASSERT_NEW(type2, ==, col_type_StringEnum);
            ...
            case col_type_StringEnum: {
                ...
                if (type2 == col_type_StringEnum) {
                    ...
                    TIGHTDB_ASSERT_NEW(type2, ==, col_type_String);
                    const AdaptiveStringColumn& c2 = t.get_column_string(i);
                    if (!c1.compare_string(c2))
                        return false;
                }
...
void Table::Verify() const
{
    ...
    // Verify row accessors
    {
        for (RowBase* row = m_row_accessors; row; row = row->m_next) {
            ...
            TIGHTDB_ASSERT_NEW(row->m_table.get(), ==, this);
            // Check that its row index is not out of bounds
            TIGHTDB_ASSERT_NEW(row->m_row_ndx, <, size());
        }
    }
    ...
    // Verify column accessors
    {
        ...
        TIGHTDB_ASSERT_NEW(n, ==, m_cols.size());
        for (size_t i = 0; i != n; ++i) {
            const ColumnBase& column = get_column_base(i);
            std::size_t ndx_in_parent = m_spec.get_column_ndx_in_parent(i);
            TIGHTDB_ASSERT_NEW(ndx_in_parent, ==, column.get_root_array()->get_ndx_in_parent());
            column.Verify(*this, i);
            TIGHTDB_ASSERT_NEW(column.size(), ==, m_size);
        }
    }
}
