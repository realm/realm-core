...
void Group::init_from_ref(ref_type top_ref) TIGHTDB_NOEXCEPT
{
    ...
    TIGHTDB_ASSERT_NEW(top_size, >=, 3);

    // Logical file size must not exceed actual file size
    TIGHTDB_ASSERT_NEW(size_t(m_top.get(2) / 2), <=, m_alloc.get_baseline());
...
void Group::reset_free_space_versions()
{
    ...
    if (m_free_versions.is_attached()) {
        TIGHTDB_ASSERT_NEW(m_top.size(), ==, 7);
    ...
    TIGHTDB_ASSERT_NEW(m_top.size(), >=, 5);
...
size_t Group::create_table(StringData name)
{
    ...
    TIGHTDB_ASSERT_NEW(ndx, ==, m_table_names.size());
...
{
    TIGHTDB_ASSERT(is_attached());
    TIGHTDB_ASSERT_NEW(m_tables.size(), ==, m_table_names.size());
    if (table_ndx >= m_tables.size())
        throw LogicError(LogicError::table_index_out_of_range);
                  ...
                  bool pad_for_encryption, uint_fast64_t version_number)
{
    ...
    TIGHTDB_ASSERT_NEW(out_2.get_pos(), ==, final_file_size);
...
void Group::commit()
{
    ...
    if (m_free_positions.is_attached()) {
        ...
        TIGHTDB_ASSERT_NEW(m_top.size(), ==, 3);
...
{
    TIGHTDB_ASSERT_NEW(new_top_ref, <, new_file_size);
    TIGHTDB_ASSERT(!is_attached());
...
public:
    ...
    {
        TIGHTDB_ASSERT_NEW(table_ndx, <=, num_tables);
...
public:
    ...
    {
        TIGHTDB_ASSERT_NEW(ref % 8, ==, 0); // 8-byte alignment
        TIGHTDB_ASSERT_NEW(size % 8, ==, 0); // 8-byte alignment
        TIGHTDB_ASSERT_NEW(size, >, 0);
        TIGHTDB_ASSERT_NEW(ref, >=, m_ref_begin);
        TIGHTDB_ASSERT_NEW(size, <=, m_immutable_ref_end - ref);
        Chunk chunk;
        chunk.ref  = ref;
        chunk.size = size;
    ...
    void add_mutable(ref_type ref, size_t size)
    {
        ...
        TIGHTDB_ASSERT_NEW(size % 8, ==, 0); // 8-byte alignment
        TIGHTDB_ASSERT_NEW(size, >, 0);
        TIGHTDB_ASSERT_NEW(ref, >=, m_immutable_ref_end);
        TIGHTDB_ASSERT_NEW(size, <=, m_mutable_ref_end - ref);
        Chunk chunk;
        chunk.ref  = ref;
        chunk.size = size;
        m_chunks.push_back(chunk);
    ...
    void canonicalize()
    {
        ...
        if (i_1 != end) {
            while (++i_2 != end) {
                ...
                TIGHTDB_ASSERT_NEW(prev_ref_end, <=, i_2->ref);
    ...
    {
        TIGHTDB_ASSERT_NEW(m_chunks.size(), ==, 1);
        TIGHTDB_ASSERT_NEW(m_chunks.front().ref, ==, m_ref_begin);
        TIGHTDB_ASSERT_NEW(m_chunks.front().size, ==, m_mutable_ref_end - m_ref_begin);
    }
private:
    struct Chunk {
        ref_type ref;
...
void Group::Verify() const
{
    ...
    // Verify tables
    {
        ...
        for (size_t i = 0; i != n; ++i) {
            ...
            TIGHTDB_ASSERT_NEW(table->get_index_in_group(), ==, i);
            table->Verify();
        }
    }
    ...
    if (m_free_positions.is_attached()) {
        size_t n = m_free_positions.size();
        TIGHTDB_ASSERT_NEW(n, ==, m_free_lengths.size());
        if (m_free_versions.is_attached())
            TIGHTDB_ASSERT_NEW(n, ==, m_free_versions.size());
        for (size_t i = 0; i != n; ++i) {
    ...
    TIGHTDB_ASSERT_NEW(immutable_ref_end, <=, baseline);
