...
template<class T, class C> Column::NodeChange ColumnBase::DoInsert(size_t ndx, T value)
{
    if (!root_is_leaf()) {
        ...
        if (nc.type == NodeChange::split) {
            ...
            const size_t preoffset = node_ndx ? to_size_t(offsets.get(node_ndx-1)) : 0;
...
template<class C> void ColumnBase::NodeInsertSplit(size_t ndx, size_t new_ref)
{
    ...
    const size_t offset = ndx ? to_size_t(offsets.get(ndx-1)) : 0;
    const size_t newSize = orig_col.size();
    const size_t newOffset = offset + newSize;
#ifdef TIGHTDB_DEBUG
    const size_t oldSize = to_size_t(offsets.get(ndx)) - offset;
#endif
