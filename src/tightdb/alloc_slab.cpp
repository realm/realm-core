...
{
    TIGHTDB_ASSERT_NEW(translate(ref), ==, addr);
    ...
    TIGHTDB_ASSERT_NEW(read_only, ||, m_free_space_state == free_space_Dirty);
...
void SlabAlloc::do_prepare_for_update(char* mutable_data, util::File::Map<char>& mapping)
{
    ...
    TIGHTDB_ASSERT_NEW(footer->m_magic_cookie, ==, footer_magic_cookie);
...
void SlabAlloc::Verify() const
{
    ...
    for (iter chunk = m_free_space.begin(); chunk != end; ++chunk) {
        ...
        TIGHTDB_ASSERT_NEW(chunk_ref_end, <=, slab_ref_end);
    }
}
