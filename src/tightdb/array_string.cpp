...
bool ArrayString::Set(size_t ndx, const char* value, size_t len)
{
    ...
    if (width > m_width) {
        ...
        while (--k >= 0) {
            ...
            memmove(data, v, oldwidth); // FIXME: Use std::copy()
    ...
    char* const end = data + m_width;
    memmove(data, value, len); // FIXME: Use std::copy()
...
bool ArrayString::Insert(size_t ndx, const char* value, size_t len)
{
    ...
    if (doExpand) {
        ...
        memmove(dst, src, count); // FIXME: Use std::copy()
    }
...
void ArrayString::Delete(size_t ndx)
{
    ...
    if (ndx < m_len) {
        ...
        memmove(dst, src, len); // FIXME: Use std::copy()
    }
