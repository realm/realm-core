...
Query::Query(Table& table, RowIndexes* tv) : m_table(table.get_table_ref()), m_tableview(tv)
{
    Create();
}

Query::Query(const Table& table, RowIndexes* tv) : m_table((const_cast<Table&>(table)).get_table_ref()), m_tableview(tv)
{
    Create();
}
