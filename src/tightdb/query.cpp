...
void Query::Create()
{
    ...
    TIGHTDB_ASSERT_NEW(first.capacity(), >, first.size()); // see above fixme
    first.push_back(0);
...
size_t Query::peek_tableview(size_t tv_index) const
{
    ...
    TIGHTDB_ASSERT_NEW(tv_index, <, m_view->size());
...
Query& Query::group()
{
    update.push_back(0);
    update_override.push_back(0);
    TIGHTDB_ASSERT_NEW(first.capacity(), >, first.size()); // see fixme in ::Create()
    first.push_back(0);
    pending_not.push_back(false);
    return *this;
}
...
size_t Query::find(size_t begin)
{
    ...
    TIGHTDB_ASSERT_NEW(begin, <=, m_table->size());
...
void Query::find_all(TableViewBase& ret, size_t start, size_t end, size_t limit) const
{
    ...
    TIGHTDB_ASSERT_NEW(start, <=, m_table->size());
