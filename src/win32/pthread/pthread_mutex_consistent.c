...
ptw32_robust_mutex_inherit(pthread_mutex_t * mutex)
{
  ...
  ptw32_robust_node_t* robust = mx.original->robustNode;
...
ptw32_robust_mutex_remove(pthread_mutex_t* mutex, ptw32_thread_t* otp)
{
  ...
  list = &(((ptw32_thread_t*)mx.original->ownerThread.p)->robustMxList);
  mx.original->ownerThread.p = otp;
  if (robust->next != NULL)
    {
