...
/* Subword parallel search
    ...
    For half the comparison functions, it is also necessary to pass in a vector where
    every bitfield is one. This can be generated by the field_bit0<width> template.
...
inline uint64_t find_all_fields_signed_LE(uint64_t MSBs, uint64_t all_ones, uint64_t A, uint64_t B)
{
    return find_all_fields_EQ(MSBs, all_ones, A, B) | find_all_fields_signed_LT(MSBs, A, B);
}
