...
void Table::set_embedded(bool embedded)
{
    ...
    if (Replication* repl = get_repl()) {
        if (repl->get_history_type() == Replication::HistoryType::hist_SyncClient) {
            throw std::logic_error(util::format("Cannot change table '%1' to embedded when using Sync.", get_name()));
        }
    }
    ...
    if (get_primary_key_column()) {
        throw std::logic_error(
            util::format("Cannot change table '%1' to embedded when using a primary key.", get_name()));
    }
    ...
    if (!has_backlink_columns) {
        throw std::logic_error(
            util::format("Cannot change table '%1' to embedded without backlink columns. Table must be embedded in "
        ...
        for (auto object : *this) {
            ...
            if (backlink_count == 0) {
                throw std::logic_error(util::format(
                    "At least one object in table '%1' does not have a backlink (data would get lost).", get_name()));
            }
            else if (backlink_count > 1) {
                throw std::logic_error(
                    util::format("At least one object in table '%1' does have multiple backlinks.", get_name()));
            }
        }
    }
...
bool Table::migrate_objects()
{
    ...
    for (size_t col_ndx = 0; col_ndx < nb_columns; col_ndx++) {
        ...
        if (!(col_ndx < col_refs.size())) {
            throw std::runtime_error(util::format("Table '%1' corrupted by previous upgrade attempt", get_name()));
        }
...
void Table::set_primary_key_column(ColKey col_key)
{
    ...
    if (Replication* repl = get_repl()) {
        if (repl->get_history_type() == Replication::HistoryType::hist_SyncClient) {
            throw std::logic_error(util::format("Cannot change pk column in table '%1' in sync client", get_name()));
        }
    }
    ...
    auto func = [&](Cluster* cluster) {
        ...
        for (size_t i = 0; i < sz; i++) {
            if (from_nullability && from_arr.is_null(i)) {
                if (throw_on_null) {
                    throw std::runtime_error(util::format("Table '%1' column '%2' has null value(s)", get_name(),
            ...
            if (ref_from) {
                ...
                for (size_t j = 0; j < n; j++) {
                    ...
                    if (!from_nullability || aggregate_operations::valid_for_agg(v)) {
                        ...
                        if (throw_on_null) {
                            throw std::runtime_error(util::format("Table '%1' column '%2' has null value(s) in list",
