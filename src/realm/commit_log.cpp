...
protected:
    ...
    static void set_log_entry_internal(ChangeSetBuffer*, const EntryHeader*, const char* log);
...
void WriteLogCollector::set_log_entry_internal(ChangeSetBuffer* entry, const EntryHeader* hdr, const char* log)
{
    *entry = ChangeSetBuffer(BinaryData(log, size_t(hdr->size)));
}
...
void WriteLogCollector::get_changesets(version_type from_version, version_type to_version,
                                       ChangeSetBuffer* logs_buffer) const noexcept
{
