...
{
    return [callback, userdata = SharedUserdata(userdata, FreeUserdata(userdata_free))](
               const App::UserAPIKey& apikey, util::Optional<AppError> error) {
        if (error) {
                                                                 ...
                                                                 realm_free_userdata_func_t userdata_free)
{
    return with_app_user(user, [&](auto& user) {
        auto cb = [callback, userdata = SharedUserdata{userdata, FreeUserdata(userdata_free)}](
                      const std::vector<App::UserAPIKey>& apikeys, util::Optional<AppError> error) {
            if (error) {
                realm_app_error_t c_error(to_capi(*error));
    ...
    return wrap_err([&] {
        collection->find_bson(convert_to_bson<bson::BsonDocument>(filter_ejson),
                              ...
                              [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                  handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                   {data, delete_data}, callback);
                              });
        return true;
    });
}
                                             ...
                                             realm_mongodb_callback_t callback)
{
    ...
    REALM_ASSERT(options);
    return wrap_err([&] {
        collection->find_one_bson(convert_to_bson<bson::BsonDocument>(filter_ejson),
                                  to_mongodb_collection_find_options(options),
                                  [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                      handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                       {data, delete_data}, callback);
                                              ...
                                              realm_mongodb_callback_t callback)
{
    REALM_ASSERT(collection);
    return wrap_err([&] {
        collection->aggregate_bson(convert_to_bson<bson::BsonArray>(filter_ejson),
                                   [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                       handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                        {data, delete_data}, callback);
                                   });
                                               ...
                                               realm_mongodb_callback_t callback)
{
    REALM_ASSERT(collection);
    return wrap_err([&] {
        collection->insert_one_bson(convert_to_bson<bson::BsonDocument>(filter_ejson),
                                    [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                        handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                         {data, delete_data}, callback);
                                    });
                                                ...
                                                realm_mongodb_callback_t callback)
{
    REALM_ASSERT(collection);
    return wrap_err([&] {
        collection->insert_many_bson(convert_to_bson<bson::BsonArray>(filter_ejson),
                                     [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                         handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                          {data, delete_data}, callback);
                                     });
                                               ...
                                               realm_mongodb_callback_t callback)
{
    REALM_ASSERT(collection);
    return wrap_err([&] {
        collection->delete_one_bson(convert_to_bson<bson::BsonDocument>(filter_ejson),
                                    [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                        handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                         {data, delete_data}, callback);
                                    });
                                                ...
                                                realm_mongodb_callback_t callback)
{
    REALM_ASSERT(collection);
    return wrap_err([&] {
        collection->delete_many_bson(convert_to_bson<bson::BsonDocument>(filter_ejson),
                                     [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                         handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                          {data, delete_data}, callback);
                                     });
                                               ...
                                               realm_mongodb_callback_t callback)
{
    ...
    return wrap_err([&] {
        ...
        collection->update_one_bson(bson_filter, bson_update, upsert,
                                    [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                        handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                         {data, delete_data}, callback);
                                    });
                                                ...
                                                realm_mongodb_callback_t callback)
{
    ...
    return wrap_err([&] {
        const auto& bson_filter = convert_to_bson<bson::BsonDocument>(filter_ejson);
        const auto& bson_update = convert_to_bson<bson::BsonDocument>(update_ejson);
        collection->update_many_bson(bson_filter, bson_update, upsert,
                                     [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                                         handle_mongodb_collection_result(std::move(bson), std::move(app_error),
                                                                          {data, delete_data}, callback);
                                     });
                                                        ...
                                                        realm_mongodb_callback_t callback)
{
    ...
    return wrap_err([&] {
        ...
        collection->find_one_and_update_bson(
            ...
            [=](util::Optional<bson::Bson> bson, util::Optional<AppError> app_error) {
                handle_mongodb_collection_result(std::move(bson), std::move(app_error), {data, delete_data},
                                                 callback);
            });
        return true;
    });
