...
{
    auto track_path_for_user = [&](const std::string& absolute_path) {
        ...
        perform_metadata_update([&](const auto& manager) {
            ...
            metadata->add_realm_file_path(absolute_path);
        });
    ...
    auto do_get_path = [&]() {
        ...
        std::string path =
            m_file_manager->realm_file_path(config.user->identity(), config.user->local_identity(), file_name);
        return path;
    };
    std::string path = do_get_path();
    track_path_for_user(path);
    return path;
}
