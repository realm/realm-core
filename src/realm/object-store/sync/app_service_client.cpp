...
void AppServiceClient::call_function(
    const std::string& name, const std::string& args_json, const util::Optional<std::string>& service_name,
    std::function<void(util::Optional<AppError>, util::Optional<std::string>)> completion_block)
{
    ...
    Request request{.method = HttpMethod::post, .url = route, .body = args.dump()};
    m_auth_request_client.do_authenticated_request(request, SyncManager::shared().get_current_user(), handler);
}
