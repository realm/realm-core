...
static void handle_count_response(util::Optional<AppError> error, util::Optional<std::string> value,
...
static void handle_delete_count_response(util::Optional<AppError> error, util::Optional<std::string> value,
                                         std::function<void(uint64_t, util::Optional<AppError>)> completion_block)
{
    if (value && !error) {
        try {
...
static void handle_update_response(
    util::Optional<AppError> error, util::Optional<std::string> value,
    ...
    if (value && !error) {
        ...
        try {
            ...
            return completion_block(
                RemoteMongoCollection::RemoteUpdateResult{matched_count, modified_count, upserted_id}, error);
        }
        catch (const std::exception& e) {
    ...
    std::function<void(Optional<std::string>, util::Optional<AppError>)> completion_block)
{
    try {
        ...
        return completion_block(util::none, AppError(make_error_code(JSONErrorCode::malformed_json),
                                                     util::format("document parse %1", e.what())));
    }
}
...
void RemoteMongoCollection::count(const std::string& filter_json, uint64_t limit,
                                  std::function<void(uint64_t, util::Optional<AppError>)> completion_block)
{
    try {
        auto base_args = m_base_operation_args;
            ...
            [completion_block](util::Optional<AppError> error, util::Optional<std::string> value) {
                if (value && !error) {
                    try {
                        ...
                        return completion_block(std::vector<std::string>(),
...
void RemoteMongoCollection::update_one(
    const std::string& filter_json, const std::string& update_json, bool upsert,
    std::function<void(RemoteMongoCollection::RemoteUpdateResult, util::Optional<AppError>)> completion_block)
{
    try {
        auto base_args = m_base_operation_args;
                                                ...
                                                std::function<void(util::Optional<AppError>)> completion_block)
{
    try {
        ...
        m_service.call_function("findOneAndDelete", args.dump(),
