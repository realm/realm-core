...
bool App::would_log(util::Logger::Level level) const
{
    if (m_logger != nullptr) {
        return m_logger->would_log(level);
    }
    return false;
}
...
{
    // Update url components based on new hostname value
    logger().debug("App: update_hostname: %1 | %2", hostname, ws_hostname);
...
{
    m_parent->logger().debug("App: register_email: %1", email);
...
void App::UsernamePasswordProviderClient::confirm_user(const std::string& token, const std::string& token_id,
                                                       UniqueFunction<void(Optional<AppError>)>&& completion)
{
    m_parent->logger().debug("App: confirm_user");
...
{
    m_parent->logger().debug("App: resend_confirmation_email: %1", email);
    m_parent->post(
...
void App::UsernamePasswordProviderClient::retry_custom_confirmation(
    const std::string& email, UniqueFunction<void(Optional<AppError>)>&& completion)
{
    m_parent->logger().debug("App: retry_custom_confirmation: %1", email);
    m_parent->post(
...
void App::UsernamePasswordProviderClient::send_reset_password_email(
    const std::string& email, UniqueFunction<void(Optional<AppError>)>&& completion)
{
    m_parent->logger().debug("App: send_reset_password_email: %1", email);
...
void App::UsernamePasswordProviderClient::reset_password(const std::string& password, const std::string& token,
                                                         ...
                                                         UniqueFunction<void(Optional<AppError>)>&& completion)
{
    m_parent->logger().debug("App: reset_password");
...
void App::UsernamePasswordProviderClient::call_reset_password_function(
    ...
    UniqueFunction<void(Optional<AppError>)>&& completion)
{
    m_parent->logger().debug("App: call_reset_password_function: %1", email);
    ...
    UniqueFunction<void(const std::shared_ptr<SyncUser>&, Optional<AppError>)>&& completion)
{
    if (would_log(util::Logger::Level::debug)) {
        ...
        logger().debug("App: log_in_with_credentials: %1", app_info);
    }
                                   ...
                                   util::UniqueFunction<void(const Response&)>&& completion)
{
    ...
    logger().debug("App: do_authenticated_request: %1 %2", httpmethod_to_string(request.method), request.url);
                               ...
                               util::UniqueFunction<void(Optional<AppError>)>&& completion)
{
    ...
    logger().debug("App: refresh_access_token: email: %1", sync_user->user_profile().email());
                        ...
                        UniqueFunction<void(Optional<Bson>&&, Optional<AppError>)>&& completion)
{
    ...
    if ( would_log(util::Logger::Level::debug)) {
        ...
        query_stg += "]";
        logger().debug("App: call_function: %1 service_name: %2 args_bson: %3", name, service_name2, query_stg);
    }
