...
void InstructionApplier::operator()(const Instruction::AddTable& instr)
{
    ...
    auto add_table = util::overloaded{
        [&](const Instruction::AddTable::PrimaryKeySpec& spec) {
            ...
            log("sync::create_table_with_primary_key(group, \"%1\", %2, \"%3\", %4);", table_name, pk_type, pk_field,
                nullable);
    ...
    auto setter = util::overloaded{
        [&](Obj& obj, ColKey col) {
            ...
            auto visitor = util::overloaded{
                [&](const ObjLink& link) {
                    if (col.get_type() == type_Mixed || col.get_type() == type_TypedLink) {
                        obj.set_any(col, link, instr.is_default);
                    }
                    else if (col.get_type() == type_Link) {
                ...
                [&](Mixed value) {
                    ...
                    else if (col.get_type() == type_Mixed || value.get_type() == col.get_type()) {
    ...
    auto callback = util::overloaded{
        [&](LstBase& list, size_t index) {
            ...
            auto inserter = util::overloaded{
                ...
                [&](const Instruction::Payload::ObjectValue&) {
                    if (col.get_type() == type_LinkList || col.get_type() == type_Link) {
                                       ...
                                       Instruction::Path::const_iterator end, const char* instr_name, F&& callback)
{
    ...
    if (col.is_list()) {
        if (auto pindex = mpark::get_if<uint32_t>(&*begin)) {
            ...
            if (col.get_type() == type_Link || col.get_type() == type_LinkList) {
                auto table = obj.get_table();
                                                    ...
                                                    F&& callback)
{
    ...
    bad_transaction_log("%1: Nested dictionaries are not supported yet");
}
