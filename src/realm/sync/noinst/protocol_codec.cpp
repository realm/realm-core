...
void ClientProtocol::make_bind_message(int protocol_version, OutputBuffer& out,
...
void ClientProtocol::make_refresh_message(OutputBuffer& out,
                                          session_ident_type session_ident,
                                          const std::string& signed_user_token)
{
...
void ClientProtocol::make_ident_message(OutputBuffer& out,
                                        session_ident_type session_ident,
...
void ClientProtocol::make_client_version_request_message(OutputBuffer& out,
                                                         session_ident_type session_ident,
                                                         SaltedFileIdent client_file_ident)
{
...
void ClientProtocol::make_state_request_message(int protocol_version, OutputBuffer& out,
                                                session_ident_type session_ident,
...
void ClientProtocol::UploadMessageBuilder::add_changeset(version_type client_version,
...
void ClientProtocol::UploadMessageBuilder::make_upload_message(int protocol_version,
                                                               OutputBuffer& out,
                                                               session_ident_type session_ident,
                                                               ...
                                                               version_type locked_server_version)
{
    ...
    if (body.size() > g_max_uncompressed) {
        compressed_body_size =
            _impl::compression::allocate_and_compress(m_compress_memory_arena,
                                                       body,
...
void ClientProtocol::make_mark_message(OutputBuffer &out,
                                       session_ident_type session_ident,
...
void ClientProtocol::make_ping(OutputBuffer &out,
                               milliseconds_type timestamp,
                               milliseconds_type rtt)
{
...
void ServerProtocol::make_ident_message(int protocol_version, OutputBuffer& out,
                                        session_ident_type session_ident,
...
/// <changeset>
void ServerProtocol::insert_single_changeset_download_message(OutputBuffer& out,
                                                              const ChangesetInfo& changeset_info,
                                                              util::Logger& logger)
{
    const sync::HistoryEntry& entry = changeset_info.entry;

    out << changeset_info.server_version << " " << changeset_info.client_version << " "
        << entry.origin_timestamp << " " << entry.origin_file_ident << " "
        << changeset_info.original_size << " " << entry.changeset.size() << " ";
    ...
    if (logger.would_log(util::Logger::Level::trace)) {
        logger.trace("DOWNLOAD: insert single changeset (server_version=%1, "
                     ...
                     changeset_info.server_version, changeset_info.client_version,
                     entry.origin_timestamp, entry.origin_file_ident,
                     changeset_info.original_size, entry.changeset.size(),
...
void ServerProtocol::make_download_message(int protocol_version, OutputBuffer& out,
                                           session_ident_type session_ident,
                                           ...
                                           bool body_is_compressed, util::Logger& logger)
{
    static_cast<void>(protocol_version);
    ...
    logger.detail("Sending: DOWNLOAD(download_server_version=%1, download_client_version=%2, "
                  ...
                  "compressed_body_size=%10)", download_server_version, download_client_version,
                  latest_server_version, latest_server_version_salt, upload_client_version,
                  upload_server_version, num_changesets, body_is_compressed,
                  uncompressed_body_size, compressed_body_size); // Throws
}


void ServerProtocol::make_unbound_message(OutputBuffer& out,
                                          session_ident_type session_ident)
{
...
void ServerProtocol::make_error_message(int protocol_version, OutputBuffer& out,
                                        sync::ProtocolError error_code, const char* message,
                                        std::size_t message_size, bool try_again,
                                        session_ident_type session_ident)
{
    static_cast<void>(protocol_version);
    sync::ProtocolError error_code_2 = error_code;
    out << "error " << int(error_code_2) << " " << message_size << " " << int(try_again) << " " <<
        session_ident << "\n"; // Throws
