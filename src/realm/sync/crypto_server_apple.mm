...
PKey PKey::load_public(const std::string& pemfile)
{
    ...
    PKey pkey;
    pkey.m_impl->public_key = load_public_from_data(static_cast<CFDataRef>(pem_data));
    return pkey;
}

PKey PKey::load_public(BinaryData pem_buffer)
{
    CFPtr<CFDataRef> pem_data = adoptCF(CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(pem_buffer.data()),
                                                                    pem_buffer.size(), kCFAllocatorNull));

    PKey pkey;
    pkey.m_impl->public_key = load_public_from_data(static_cast<CFDataRef>(pem_data.get()));
    return pkey;
}
...
bool PKey::verify(BinaryData message, BinaryData signature) const
{
    ...
    CFPtr<CFDataRef> signatureCF = adoptCF(CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(signature.data()),
                                                                       signature.size(), kCFAllocatorNull));
    CFPtr<CFDataRef> messageCF = adoptCF(CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(message.data()),
    ...
    auto translateError = [](CFErrorRef error) -> CryptoError {
        ...
        return CryptoError(std::string("Error verifying message: ") + [static_cast<id>(error) description].UTF8String);
    };

    CFErrorRef error = nullptr;
    CFPtr<SecTransformRef> verifier = adoptCF(SecVerifyTransformCreate(m_impl->public_key.get(), signatureCF.get(), &error));
    if (!verifier) {
        throw translateError(error);
    }

    if (!SecTransformSetAttribute(verifier.get(), kSecTransformInputAttributeName, messageCF.get(), &error) ||
        !SecTransformSetAttribute(verifier.get(), kSecDigestTypeAttribute, kSecDigestSHA2, &error) ||
        !SecTransformSetAttribute(verifier.get(), kSecDigestLengthAttribute, @256, &error)) {
        throw translateError(error);
    }

    CFPtr<CFTypeRef> result = adoptCF(SecTransformExecute(verifier.get(), &error));
    if (!result) {
        throw translateError(error);
    }
    ...
    OSStatus status = SecKeyRawVerify(m_impl->public_key.get(), (SecPadding)kSecPaddingPKCS1SHA256,
