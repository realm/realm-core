...
void parse_arguments(int argc, char* argv[], Configuration& configuration)
{
    ...
    static const char* opt_desc =
        "r:L:p:J:M:i:d:N:l:YPk:m:hnsC:K:b:DSu:t:f:H:I:qe:jRGEa:g:U:BA12:v:x:o:cOQF:";
    ...
    if (optind != argc) {
        std::cerr << "Error: This command does not support positional arguments (e.g.: `"
                  << argv[optind] << "')\n\n";
        show_help(argv[0]);
        std::exit(EXIT_FAILURE);
    }
                                       ...
                                       const util::Optional<std::array<char, 64>>& encryption_key)
{
    ...
    if (file_exist) {
        ...
        if (!verified) {
            ...
            if (encryption_key) {
                os <<
                    "The server was started with an encryption key that "
                    "does not match the fingerprint file '" <<
                    fingerprint_path << "'. The reason for the mismatch "
                ...
                os <<
                    "The server was started without an encryption key. "
        ...
        const std::string fingerprint =
            encrypt::calculate_fingerprint(encryption_key);
...
auto sync::load_client_file_blacklists(const config::Configuration& config,
                                       util::Logger& logger) -> Server::ClientFileBlacklists
{
    ...
    if (found_file) {
        logger.info("Loaded %1 client file blacklists from '%2' (%3 client files in total)",
                    lists.size(), path, num_idents); // Throws
    }
    else {
