...
struct MergeUtils {
    ...
    // 'next_element' represents the next element in the right path if the left
    // path is a prefix of the right path.

    bool is_prefix_of(const Instruction::EraseObject&, const Instruction::TableInstruction&) const noexcept
    {
        // Right side is a schema instruction.
        return false;
    }

    bool is_prefix_of(const Instruction::EraseObject& left, const Instruction::PathInstruction& right) const noexcept
    {
...
DEFINE_NESTED_MERGE(Instruction::Update)
{
    ...
    if (outer.value.type == Type::ObjectValue) {
        // Creating an embedded object is an idempotent operation, and should not eliminate updates to the subtree.
        return;
    }
    ...
    if (auto next_element = is_prefix_of(outer, inner); next_element) {
