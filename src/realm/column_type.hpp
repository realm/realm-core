/*************************************************************************
 *
 * REALM CONFIDENTIAL
 * __________________
 *
 *  [2011] - [2012] Realm Inc
 *  All Rights Reserved.
 *
 * NOTICE:  All information contained herein is, and remains
 * the property of Realm Incorporated and its suppliers,
 * if any.  The intellectual and technical concepts contained
 * herein are proprietary to Realm Incorporated
 * and its suppliers and may be covered by U.S. and Foreign Patents,
 * patents in process, and are protected by trade secret or copyright law.
 * Dissemination of this information or reproduction of this material
 * is strictly forbidden unless prior written permission is obtained
 * from Realm Incorporated.
 *
 **************************************************************************/
#ifndef REALM_COLUMN_TYPE_HPP
#define REALM_COLUMN_TYPE_HPP

namespace realm {


// Note: Enumeration value assignments must be kept in sync with
// <realm/data_type.hpp>.
enum ColumnType {
    // Column types
    column_type_Int         =  0,
    column_type_Bool        =  1,
    column_type_String      =  2,
    column_type_StringEnum  =  3, // double refs
    column_type_Binary      =  4,
    column_type_Table       =  5,
    column_type_Mixed       =  6,
    column_type_DateTime    =  7,
    column_type_Reserved1   =  8, // new date
    column_type_Float       =  9,
    column_type_Double      = 10,
    column_type_Reserved4   = 11, // Decimal
    column_type_Link        = 12,
    column_type_LinkList    = 13,
    column_type_BackLink    = 14
};


// Column attributes can be combined using bitwise or.
enum ColumnAttr {
    column_attr_None = 0,
    column_attr_Indexed = 1,

    /// Specifies that this column forms a unique constraint. It requires
    /// `column_attr_Indexed`.
    column_attr_Unique = 2,

    /// Specifies that this column forms the primary key. It implies a non-null
    /// constraint on the column, and it requires `column_attr_Unique`.
    column_attr_PrimaryKey = 4,

    /// Specifies that the links of this column are strong, not weak. Applies
    /// only to link columns (`type_Link` and `type_LinkList`).
    column_attr_StrongLinks = 8,

    /// Specifies that elements in the column can be null.
    column_attr_Nullable = 16
};


} // namespace realm

#endif // REALM_COLUMN_TYPE_HPP
