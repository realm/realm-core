...
template<class T>
inline void Buffer<T>::set_size(std::size_t new_size)
{
...
template<class T>
inline void Buffer<T>::resize(std::size_t new_size, std::size_t copy_begin,
...
template<class T>
inline void Buffer<T>::reserve(std::size_t used_size,
...
template<class T>
inline void Buffer<T>::reserve_extra(std::size_t used_size,
...
template<class T>
inline T* Buffer<T>::release() REALM_NOEXCEPT
{
    m_size = 0;
...
template<class T>
inline AppendBuffer<T>::AppendBuffer() REALM_NOEXCEPT: m_size(0)
{
}

template<class T>
inline std::size_t AppendBuffer<T>::size() const REALM_NOEXCEPT
{
    return m_size;
}

template<class T>
inline T* AppendBuffer<T>::data() REALM_NOEXCEPT
{
    return m_buffer.data();
}

template<class T>
inline const T* AppendBuffer<T>::data() const REALM_NOEXCEPT
{
    return m_buffer.data();
}

template<class T>
inline void AppendBuffer<T>::append(const T* data, std::size_t size)
{
...
template<class T>
inline void AppendBuffer<T>::reserve(std::size_t min_capacity)
{
    m_buffer.reserve(m_size, min_capacity);
}

template<class T>
inline void AppendBuffer<T>::resize(std::size_t size)
{
    reserve(size);
    m_size = size;
}
...
template<class T>
inline void AppendBuffer<T>::clear() REALM_NOEXCEPT
{
    m_size = 0;
}
