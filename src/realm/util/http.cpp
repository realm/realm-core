...
bool HTTPParserBase::parse_header_line(size_t len)
{
    ...
    if (colon == end) {
        m_logger.error("Bad header line in HTTP message:\n%1", line);
        return false;
    }
    ...
    if (key == "Content-Length") {
        if (value.size() == 0) {
            ...
            // A warning is logged.
            m_logger.warn("Empty Content-Length header in HTTP message:\n%1", line);
            ...
            if (ss >> content_length && ss.eof()) {
                ...
                m_logger.error("Bad Content-Length header in HTTP message:\n%1", line);
                return false;
            }
        }
    }
