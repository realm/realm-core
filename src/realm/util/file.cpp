...
std::string make_temp_dir()
{
    ...
    stdx::filesystem::path temp = stdx::filesystem::temp_directory_path();
    ...
    for (;;) {
        ...
        path = buffer;
        stdx::filesystem::remove(path);
        try {
            stdx::filesystem::create_directory(path);
            break;
        }
        catch (const stdx::filesystem::filesystem_error& ex) {
...
bool File::is_dir(const std::string& path)
{
    ...
    std::wstring w_path = string_to_wstring(path);
    return stdx::filesystem::is_directory(w_path);
#else
...
#elif REALM_HAVE_STD_FILESYSTEM
    stdx::filesystem::path path_(path.empty() ? "." : path);
    if (path_.is_absolute())
        return path;

    return (stdx::filesystem::path(base_dir) / path_).u8string();
#else
    static_cast<void>(path);
...
{
    try {
        m_iterator = stdx::filesystem::directory_iterator(path);
    }
    catch (const stdx::filesystem::filesystem_error& e) {
...
bool DirScanner::next(std::string& name)
{
    const stdx::filesystem::directory_iterator end;
    if (m_iterator != end) {
