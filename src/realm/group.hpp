...
public:
    ...
    TableRef insert_table(std::size_t idx, StringData name, bool require_unique_name = true);
    TableRef get_or_add_table(StringData name, bool* was_added = 0);
    TableRef get_or_insert_table(std::size_t index, StringData name, bool* was_added = nullptr);
    ...
    template<class T> BasicTableRef<T> insert_table(std::size_t index, StringData name,
                                                    bool require_unique_name = true);
    template<class T> BasicTableRef<T> get_or_add_table(StringData name, bool* was_added = 0);
    template<class T> BasicTableRef<T> get_or_insert_table(std::size_t index, StringData name,
    ...
    void move_table(std::size_t index_1, std::size_t index_2);
...
private:
    ...
    Table* do_insert_table(std::size_t, StringData name, DescSetter desc_setter, bool require_unique_name);
    Table* do_insert_table(std::size_t, StringData name, DescSetter desc_setter);
    Table* do_get_or_add_table(StringData name, DescMatcher desc_matcher, DescSetter setter,
                               bool* was_added);
    Table* do_get_or_insert_table(std::size_t, StringData name, DescMatcher desc_matcher,
                                  DescSetter desc_setter, bool* was_added);

    void create_and_insert_table(std::size_t new_table_ndx, StringData name);
...
inline TableRef Group::insert_table(std::size_t table_ndx, StringData name, bool require_unique_name)
{
    if (!is_attached())
        throw LogicError(LogicError::detached_accessor);
...
inline TableRef Group::get_or_insert_table(std::size_t table_ndx, StringData name, bool* was_added)
{
    if (!is_attached())
        throw LogicError(LogicError::detached_accessor);
    DescMatcher desc_matcher = nullptr; // Do not check descriptor
...
template<class T>
inline BasicTableRef<T> Group::insert_table(std::size_t table_ndx, StringData name,
...
template<class T> BasicTableRef<T> Group::get_or_insert_table(std::size_t table_ndx,
                                                              StringData name,
                                                              bool* was_added)
{
    REALM_STATIC_ASSERT(IsBasicTable<T>::value, "Invalid table type");
...
public:
    ...
    static Table& insert_table(Group& group, std::size_t table_ndx, StringData name,
                               bool require_unique_name)
    {
    ...
    static Table& get_or_insert_table(Group& group, std::size_t table_ndx, StringData name,
                                      bool* was_inserted)
    {
