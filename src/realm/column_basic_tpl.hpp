...
template<class T>
inline size_t BasicColumn<T>::size() const noexcept
{
    if (root_is_leaf())
        return m_array->size();
...
template<class T>
T BasicColumn<T>::get(size_t ndx) const noexcept
{
...
template<class T>
inline void BasicColumn<T>::add(T value)
{
...
template<class T>
inline void BasicColumn<T>::insert(size_t row_ndx, T value)
{
...
template<class T>
inline void BasicColumn<T>::erase(size_t row_ndx)
{
...
template<class T>
inline void BasicColumn<T>::move_last_over(size_t row_ndx)
{
    size_t last_row_ndx = size() - 1; // Note that size() is slow
...
template<class T>
inline void BasicColumn<T>::swap_rows(size_t row_ndx_1, size_t row_ndx_2)
{
    do_swap_rows(row_ndx_1, row_ndx_2);
}

template<class T>
inline void BasicColumn<T>::clear()
{
    do_clear(); // Throws
}
...
template<class T>
void BasicColumn<T>::erase(size_t row_ndx, bool is_last)
{
    REALM_ASSERT_3(row_ndx, <, size());
...
template<class T>
void BasicColumn<T>::do_move_last_over(size_t row_ndx, size_t last_row_ndx)
{
...
template<class T>
class BasicColumn<T>::CreateHandler: public ColumnBase::CreateHandler {
public:
...
template<class T>
ref_type BasicColumn<T>::create(Allocator& alloc, size_t size)
{
...
template<class T>
class BasicColumn<T>::SliceHandler: public ColumnBase::SliceHandler {
public:
...
template<class T>
ref_type BasicColumn<T>::write(size_t slice_offset, size_t slice_size, size_t table_size,
...
// Implementing pure virtual method of ColumnBase.
template<class T>
void BasicColumn<T>::clear(size_t, bool)
{
    do_clear(); // Throws
}
...
template<class T>
void BasicColumn<T>::refresh_accessor_tree(size_t, const Spec&)
{
...
template<class T>
size_t BasicColumn<T>::verify_leaf(MemRef mem, Allocator& alloc)
{
...
template<class T>
void BasicColumn<T>::leaf_to_dot(MemRef leaf_mem, ArrayParent* parent, size_t ndx_in_parent,
...
template<class T>
size_t BasicColumn<T>::find_first(T value, size_t begin, size_t end) const
{
...
template<class T>
void BasicColumn<T>::find_all(IntegerColumn &result, T value, size_t begin, size_t end) const
{
    REALM_ASSERT_3(begin, <=, size());
    REALM_ASSERT(end == npos || (begin <= end && end <= size()));
...
{
    return size_t(aggregate<T, int64_t, act_Count, Equal>(*this, target, 0, size(), npos, nullptr));
}

template<class T>
typename BasicColumn<T>::SumType BasicColumn<T>::sum(size_t begin, size_t end,
    size_t limit, size_t* return_ndx) const
{
    return aggregate<T, SumType, act_Sum, NotNull>(*this, 0, begin, end, limit, return_ndx);
}
template<class T>
T BasicColumn<T>::minimum(size_t begin, size_t end, size_t limit, size_t* return_ndx) const
{
    return aggregate<T, T, act_Min, NotNull>(*this, 0, begin, end, limit, return_ndx);
}

template<class T>
T BasicColumn<T>::maximum(size_t begin, size_t end, size_t limit, size_t* return_ndx) const
{
    return aggregate<T, T, act_Max, NotNull>(*this, 0, begin, end, limit, return_ndx);
}

template<class T>
double BasicColumn<T>::average(size_t begin, size_t end, size_t limit, size_t* return_ndx) const
{
    if (end == size_t(-1))
        end = size();
...
template<class T>
void BasicColumn<T>::do_insert(size_t row_ndx, T value, size_t num_rows)
{
...
template<class T>
inline size_t BasicColumn<T>::lower_bound(T value) const noexcept
{
    if (root_is_leaf()) {
...
template<class T>
inline size_t BasicColumn<T>::upper_bound(T value) const noexcept
{
    if (root_is_leaf()) {
