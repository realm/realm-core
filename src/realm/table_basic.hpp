...
public:
    ...
    template<class L>
    void insert(std::size_t i, const util::Tuple<L>& tuple)
    {
        insert(i);
        set(i, tuple);
    }
    ...
    template<class L>
    void set(std::size_t i, const util::Tuple<L>& tuple)
    {
    ...
    template<class T>
    friend bool is_a(const Table&) REALM_NOEXCEPT;

    //@{
    /// These functions return null if the specified table is not
    /// compatible with the specified table type.
    template<class T>
    friend BasicTableRef<T> checked_cast(TableRef) REALM_NOEXCEPT;
    template<class T>
    friend BasicTableRef<const T> checked_cast(ConstTableRef) REALM_NOEXCEPT;
...
private:
    ...
    Subtab* get_subtable_ptr(std::size_t col_idx, std::size_t row_idx)
    {
        return static_cast<Subtab*>(Table::get_subtable_ptr(col_idx, row_idx));
    }

    template<class Subtab>
    const Subtab* get_subtable_ptr(std::size_t col_idx, std::size_t row_idx) const
    {
...
struct AssignIntoCol<int64_t, col_idx> {
    template<class L>
    static void exec(Table* t, std::size_t row_idx, util::Tuple<L> tuple)
    {
...
template<class T>
inline bool is_a(const Table& t) REALM_NOEXCEPT
{
...
template<class T>
inline BasicTableRef<T> checked_cast(TableRef t) REALM_NOEXCEPT
{
    if (!is_a<T>(*t))
...
template<class T>
inline BasicTableRef<const T> checked_cast(ConstTableRef t) REALM_NOEXCEPT
{
    if (!is_a<T>(*t))
