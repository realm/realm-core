...
void StringEnumColumn::do_insert(size_t row_ndx, StringData value, size_t num_rows)
{
    ...
    if (m_string_index) {
        ...
        m_string_index->insert(row_ndx_2, value, num_rows, is_append); // Throws
    }
}
...
void StringEnumColumn::do_insert(size_t row_ndx, StringData value, size_t num_rows, bool is_append)
{
    ...
    if (m_string_index)
        m_string_index->insert(row_ndx, value, num_rows, is_append); // Throws
}
...
void StringEnumColumn::do_erase(size_t ndx, bool is_last)
{
    ...
    //  position to update (as it looks for the old value))
    if (m_string_index)
        m_string_index->erase<StringData>(ndx, is_last);
...
void StringEnumColumn::do_move_last_over(size_t row_ndx, size_t last_row_ndx)
{
    ...
    if (m_string_index) {
        ...
        m_string_index->erase<StringData>(row_ndx, is_last); // Throws

        // update index to point to new location
        if (row_ndx != last_row_ndx) {
            StringData moved_value = get(last_row_ndx);
            m_string_index->update_ref(moved_value, last_row_ndx, row_ndx); // Throws
        }
    }
...
{
    if (m_string_index)
        return m_string_index->count(value);
...
FindRes StringEnumColumn::find_all_no_copy(StringData value, InternalFindResult& result) const
{
    ...
    REALM_ASSERT(m_string_index);

    return m_string_index->find_all_no_copy(value, result);
}


void StringEnumColumn::find_all(IntegerColumn& res, StringData value, size_t begin, size_t end) const
{
    if (m_string_index && begin == 0 && end == size_t(-1))
        return m_string_index->find_all(res, value);
...
{
    if (m_string_index && begin == 0 && end == npos)
        return m_string_index->find_first(value);
...
SearchIndex* StringEnumColumn::create_search_index()
{
    REALM_ASSERT(!m_string_index);
    ...
    m_string_index = std::move(index);
    return m_string_index.get();
}

void StringEnumColumn::destroy_search_index() noexcept
{
    m_string_index.reset();
}
...
void StringEnumColumn::install_search_index(std::unique_ptr<StringIndex> index) noexcept
{
    ...
    m_string_index = std::move(index); // we now own this index
}
...
void StringEnumColumn::refresh_accessor_tree(size_t col_ndx, const Spec& spec)
{
    ...
    if (m_string_index) {
        ...
        m_string_index->set_ndx_in_parent(ndx_in_parent + 1);
        m_string_index->refresh_accessor_tree(col_ndx, spec); // Throws
    }
}
...
void StringEnumColumn::verify(const Table& table, size_t col_ndx) const
{
    ...
    REALM_ASSERT_3(column_has_search_index, ==, bool(m_string_index));
    if (column_has_search_index) {
        REALM_ASSERT_3(m_string_index->get_ndx_in_parent(), ==, get_root_array()->get_ndx_in_parent() + 1);
    }
}
...
void StringEnumColumn::do_dump_node_structure(std::ostream& out, int level) const
{
    ...
    out << std::setw(indent) << ""
        << "Search index\n";
    m_string_index->do_dump_node_structure(out, level + 1);
}
