...
struct EitherIsString {
    typedef T type;
};

template<class T>
struct EitherIsString<T, StringData>{
         ...
         template<class L, class Cond, class R> Query create(L left, const Subexpr2<R>&right)
         {
...
#endif
             {
                 ...
                 return *new Compare<Cond, typename Common<L, R>::type>(*new Value<L>(left),
                                                                        const_cast<Subexpr2<R>&>(right).clone(), true,
                                                                        compare_string);
             }
         }
...
public:
             ...
             {
                 return *new Operator<Plus<CommonType >>
                 (static_cast<Subexpr2<L>&>(*this).clone(), *new Value<R>(right), true);
             }
             Operator<Minus<CommonType >>& operator - (R right)
             {
                 return *new Operator<Minus<CommonType >>
                 (static_cast<Subexpr2<L>&>(*this).clone(), *new Value<R>(right), true);
             }
             Operator<Mul<CommonType >>& operator * (R right)
             {
                 return *new Operator<Mul<CommonType >>
                 (static_cast<Subexpr2<L>&>(*this).clone(), *new Value<R>(right), true);
             }
             Operator<Div<CommonType >>& operator / (R right)
             {
                 return *new Operator<Div<CommonType >>
                 (static_cast<Subexpr2<L>&>(*this).clone(), *new Value<R>(right), true);
             }
             ...
             template<class Cond> Query create2 (const Subexpr2<R>&right)
             {
                 ...
                 if (left_col && right_col &&
                     std::is_same<L, R>::value && !left_col->m_nullable && !right_col->m_nullable)
                 {
...
// consider if it's simpler/better to remove this class completely and just list all 100 overloads manually anyway.
         template<class T> class Subexpr2 : public Subexpr,
         public Overloads<T, const char*>, public Overloads<T, int>, public
         Overloads<T,
                   float>, public Overloads<T, double>, public Overloads<T, int64_t>, public Overloads<T, StringData>,
...
#define RLM_U(o) \
    RLM_U2(int, o) RLM_U2(float, o) RLM_U2(double, o) RLM_U2(int64_t, o) RLM_U2(StringData, o) RLM_U2( \
        bool, \
        o) RLM_U2(DateTime, o) RLM_U2(null, o)
...
public:
             ...
             template<class TOperator> REALM_FORCEINLINE void fun(const Value * left, const Value * right)
             {
                 ...
                 for (size_t t = 0; t < vals; t++) {
                     if (std::is_same<T,
                                      int64_t>::value && (left->m_storage.is_null(t) || right->m_storage.is_null(t)))
...
// Arithmetic
         template<class R> Operator<Plus<typename Common<R, double>::type >>& operator + (double             left,
                                                                                          const Subexpr2<R>& right)
         {
             return *new Operator<Plus<
             typename Common<R,
                             double>::type >>
             (*new Value<double>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Plus<typename Common<R, float>::type >>& operator + (float              left,
                                                                                         const Subexpr2<R>& right)
         {
             return *new Operator<Plus<
             typename Common<R,
                             float>::type >> (*new Value<float>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Plus<typename Common<R, int>::type >>& operator + (int                left,
                                                                                       const Subexpr2<R>& right)
         {
             return *new Operator<Plus<
             typename Common<R, int>::type >> (*new Value<int>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Plus<typename Common<R, int64_t>::type >>& operator + (int64_t            left,
                                                                                           const Subexpr2<R>& right)
         {
             return *new Operator<Plus<
             typename Common<R,
                             int64_t>::type >>
             (*new Value<int64_t>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Minus<typename Common<R, double>::type >>& operator - (double             left,
                                                                                           const Subexpr2<R>& right)
         {
         ...
         template<class R> Operator<Minus<typename Common<R, float>::type >>& operator - (float              left,
                                                                                          const Subexpr2<R>& right)
         {
             return *new Operator<Minus<
             typename Common<R,
         ...
         template<class R> Operator<Minus<typename Common<R, int>::type >>& operator - (int                left,
                                                                                        const Subexpr2<R>& right)
         {
             return *new Operator<Minus<
             typename Common<R, int>::type >> (*new Value<int>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Minus<typename Common<R, int64_t>::type >>& operator - (int64_t            left,
                                                                                            const Subexpr2<R>& right)
         {
             return *new Operator<Minus<
             typename Common<R,
         ...
         template<class R> Operator<Mul<typename Common<R, double>::type >>& operator * (double             left,
                                                                                         const Subexpr2<R>& right)
         {
         ...
         template<class R> Operator<Mul<typename Common<R, float>::type >>& operator * (float              left,
                                                                                        const Subexpr2<R>& right)
         {
             return *new Operator<Mul<
             typename Common<R,
         ...
         template<class R> Operator<Mul<typename Common<R, int>::type >>& operator * (int                left,
                                                                                      const Subexpr2<R>& right)
         {
             return *new Operator<Mul<
             typename Common<R, int>::type >> (*new Value<int>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Mul<typename Common<R, int64_t>::type >>& operator * (int64_t            left,
                                                                                          const Subexpr2<R>& right)
         {
             return *new Operator<Mul<
             typename Common<R,
         ...
         template<class R> Operator<Div<typename Common<R, double>::type >>& operator / (double             left,
                                                                                         const Subexpr2<R>& right)
         {
         ...
         template<class R> Operator<Div<typename Common<R, float>::type >>& operator / (float              left,
                                                                                        const Subexpr2<R>& right)
         {
             return *new Operator<Div<
             typename Common<R,
         ...
         template<class R> Operator<Div<typename Common<R, int>::type >>& operator / (int                left,
                                                                                      const Subexpr2<R>& right)
         {
             return *new Operator<Div<
             typename Common<R, int>::type >> (*new Value<int>(left), const_cast<Subexpr2<R>&>(right).clone(), true);
         }
         template<class R> Operator<Div<typename Common<R, int64_t>::type >>& operator / (int64_t            left,
                                                                                          const Subexpr2<R>& right)
         {
             return *new Operator<Div<
             typename Common<R,
...
private:
         ...
         template<class T, class S, class I> Query string_compare(const Columns<StringData>&left, T right,
                                                                  bool case_insensitive);
         template<class S, class I> Query string_compare(const Columns<StringData>&left,
                                                         const Columns<StringData>&right, bool case_insensitive);
...
public:
             ...
             Columns(size_t column,
                     const Table * table): m_table_linked_from(nullptr), m_table(nullptr), m_column(column)
             {
                 m_table = table;
             }
         ...
         template<class S, class I> Query string_compare(const Columns<StringData>&left,
                                                         const Columns<StringData>&right, bool case_sensitive)
         {
             ...
             Query is_null() {
                 if (m_link_map.m_link_columns.size() > 1)
                     throw std::runtime_error(
                         "Cannot find null-links in a linked-to table (link()...is_null() not supported).");
...
private:
             ...
             explicit Columns(size_t column): m_table(nullptr) {
                 static_cast<void>(column);
             }
                 ...
                 if (m_link_map.m_link_columns.size() > 0) {
                     ...
                     if (std::is_same<T, int64_t>::value && index + ValueBase::default_size <= sgc->m_leaf_end) {
                         ...
                         sgc->m_leaf_ptr->get_chunk(index - sgc->m_leaf_start,
                                                    static_cast<Value<int64_t>*>(static_cast<ValueBase*>(&v))->
                                                    m_storage.m_first);
                         ...
                         {
                             v.m_storage.m_null =
                                 reinterpret_cast<const ArrayIntNull*>(sgc->m_leaf_ptr)->null_value();
                         }
...
public:
             ...
             Operator(TLeft & left, const TRight &right,
                      bool auto_delete = false): m_left(left), m_right(const_cast<TRight&>(right))
             {
...
public:
             ...
             Compare(TLeft & left, const TRight &right, bool auto_delete = false, const char* compare_string =
                         nullptr):
