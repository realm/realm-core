...
public:
    ...
    /// Create an object with key. If the key is omitted, a key will be
    /// generated by the system.
    ///
    /// The provided key will be converted to a GlobalKey, and is subject to
    /// tombstone resurrection. For this reason, the upper two bits of the
    /// ObjKey (used internally for tracking dangling links and object IDs with
    /// hash collisions) must not be set, except when passing a null Objkey.
    ///
    /// Note: As opposed to `create_object(GlobalKey)`, this function is not
    /// idempotent, and throws an error if an object with the corresponding key
    /// already exists.
    Obj create_object(ObjKey key = {}, const FieldValues& = {});
    /// Create an object with specific GlobalKey. If an object with that key
    /// already exists, that object is returned, and field values passed to this
    ...
    /// for that primary key, in which case the field values passed to this
    /// function are ignored.
...
private:
    ...
    Obj create_object_internal(ObjKey, const FieldValues& = FieldValues{});
