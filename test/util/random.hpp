...
/// instance of `Random`.
template<class T> T random_int(T min, T max) noexcept;

/// Same as `random_int(lim::min(), lim::max())` where `lim` is
/// `std::numeric_limits<T>`.
template<class T> T random_int() noexcept;
...
public:
    ...
    template<class T> T draw_float(T a, T b) noexcept;

    /// Same as draw_float(T(0), T(1)).
    template<class T> T draw_float() noexcept;
    ...
    template<class T> T draw_int_bits(int bits) noexcept;
...
template<class T> inline T Random::draw_float(T a, T b) noexcept
{
    return std::uniform_real_distribution<T>(a,b)(m_engine);
}

template<class T> inline T Random::draw_float() noexcept
{
    return draw_float(T(0), T(1));
}

template<class T> inline T Random::draw_int(T min, T max) noexcept
{
...
template<class T> inline T Random::draw_int() noexcept
{
    typedef std::numeric_limits<T> lim;
    return draw_int(lim::min(), lim::max());
}

template<class T> inline T Random::draw_int_max(T max) noexcept
{
    return draw_int(T(), max);
}

template<class T> inline T Random::draw_int_mod(T module_size) noexcept
{
    return draw_int_max(T(module_size-1));
}

template<class T> inline T Random::draw_int_bits(int bits) noexcept
{
    if (bits <= 0)
        return T();
    T bit = T(1) << (bits-1);
...
template<class T> inline T random_int(T min, T max) noexcept
{
...
template<class T> inline T random_int() noexcept
{
    typedef std::numeric_limits<T> lim;
