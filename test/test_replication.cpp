...
//    replay_log = &cout;
    ...
    auto perform_change = [&](std::function<void (Table&)> func) {
        ...
        // pointing to the corresponding row in `target`
        {
            ...
            target_w[0].set_int(0, 1);
            target_w[1].set_int(0, 2);
    ...
    CHECK_EQUAL(target_row_0.get_int(0), 1);
    ...
    // Break link by explicit row removal, with remaining row moved
    perform_change([](Table& origin) {
        origin[0].move_last_over();
    });
    CHECK(target_row_0 && !target_row_1);
    CHECK_EQUAL(target->size(), 1);
    CHECK_EQUAL(target_row_0.get_int(0), 2);
...
TEST(Replication_CascadeRemove_ColumnLinkList)
{
    SHARED_GROUP_TEST_PATH(path_1);
    SHARED_GROUP_TEST_PATH(path_2);
...
//    replay_log = &cout;
    ...
    auto perform_change = [&](std::function<void (Table&)> func) {
        ...
        // Apply the reset to the initial state separately from the change being
        // tested
        sg.end_read();
        repl.replay_transacts(sg, replay_log);
        {
