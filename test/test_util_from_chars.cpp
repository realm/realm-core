...
TEST(Util_FromChars_Base10)
{
    ...
    uint8_t byte = 0;
    FromCharsResult res = from_chars(&okay_byte.front(), &okay_byte.back(), byte, 10);
    CHECK_EQUAL(byte, 254);
    CHECK(res.ec == std::errc{});
    CHECK_EQUAL(res.ptr, okay_byte.data() + 3);

    res = from_chars(&overflow_byte.front(), &overflow_byte.back(), byte, 10);
    CHECK_EQUAL(byte, 254);
    CHECK(res.ec == std::errc::result_out_of_range);
    CHECK_EQUAL(res.ptr, overflow_byte.data() + 3);

    int16_t short_val;
    res = from_chars(&overflow_byte.front(), &overflow_byte.back(), short_val, 10);
    CHECK_EQUAL(short_val, 260);
    CHECK(res.ec == std::errc{});
    CHECK_EQUAL(res.ptr, overflow_byte.data() + 3);

    res = from_chars(&not_a_number.front(), &not_a_number.back(), byte, 10);
    CHECK_EQUAL(byte, 254);
    CHECK(res.ec == std::errc::invalid_argument);
    CHECK_EQUAL(res.ptr, not_a_number.data());

    int8_t signed_byte = 0;
    res = from_chars(&okay_byte.front(), &okay_byte.back(), signed_byte, 10);
    CHECK_EQUAL(signed_byte, 0);
    CHECK(res.ec == std::errc::result_out_of_range);
    CHECK_EQUAL(res.ptr, okay_byte.data() + 3);

    res = from_chars(&negative_byte.front(), &negative_byte.back(), signed_byte, 10);
...
TEST(Util_FromChars_Base16)
{
    ...
    uint8_t byte = 0;
    FromCharsResult res = from_chars(&okay_byte.front(), &okay_byte.back(), byte, 16);
    CHECK_EQUAL(byte, 255);
    CHECK(res.ec == std::errc{});
    CHECK_EQUAL(res.ptr, okay_byte.data() + 2);

    res = from_chars(&overflow_byte.front(), &overflow_byte.back(), byte, 16);
    CHECK_EQUAL(byte, 255);
    CHECK(res.ec == std::errc::result_out_of_range);
    CHECK_EQUAL(res.ptr, overflow_byte.data() + 2);

    int16_t short_val;
    res = from_chars(&overflow_byte.front(), &overflow_byte.back(), short_val, 16);
