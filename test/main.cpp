...
const char* legend = 
	...
	"\n"
	" -a: Use the first N rows to auto-detect scheme (default =10000)\n"
	...
	" -f: Overwrite .tightdb file if existing (default is to abort)\n"
	"\n"
	"Examples:\n"
...
int main(int argc, char* argv[])
{
	...
	abort2(!force_flag && File::exists(argv[argc - 1]), "Destination file '%s' already exists. Delete the file and try again", argv[argc - 1]);

	if(File::exists(argv[argc - 1]))
		File::remove(argv[argc - 1]);
	...
	try {
		if(scheme.size() > 0) {
			...
			// Auto detection
			imported_rows = importer.import_csv_auto(in_file, table, auto_detection_flag ? auto_detection_flag : 10000, empty_as_string_flag, import_rows_flag ? import_rows_flag : static_cast<size_t>(-1));
		}
		else {
	...
	if(!quiet_flag)
		cout << "\nImported " << imported_rows << " rows into table named 'table'\n";
