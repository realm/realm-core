...
TEST_TYPES(Query_IntCompressed_Value_Present, Equal, NotEqual, Less, LessEqual, Greater, GreaterEqual)
{
    TEST_TYPE c;
    ...
    char query_str[20];
    snprintf(query_str, 20, "id %s %s", c.description().c_str(), std::to_string(key).c_str());
    CHECK_EQUAL(t->query(query_str).count(), num_matches);
}

TEST_TYPES(Query_IntCompressed_Value_Not_Present, Equal, NotEqual, Less, LessEqual, Greater, GreaterEqual)
{
    TEST_TYPE c;
    size_t num_matches = 0;
    SHARED_GROUP_TEST_PATH(path);
