...
TEST(Group_ChangeEmbeddedness)
{
    ...
    CHECK_EQUAL(message, "At least one object in table 'table' does not have a backlink (data would get lost).");
    ...
    CHECK_THROW_ANY_GET_MESSAGE(t->set_embedded(true), message);
    CHECK_EQUAL(message, "At least one object in table 'table' does have multiple backlinks.");
    CHECK_NOT(t->is_embedded());
}
