...
TEST_CASE("SyncSession: management by SyncUser", "[sync][local][session]") {
...
TEST_CASE("sync: log-in", "[sync][local][session]") {
    if (!EventLoop::has_implementation())
        return;
...
TEST_CASE("SyncSession: close() API", "[sync][local][session]") {
    TestSyncManager init_sync_manager;
    auto app = init_sync_manager.app();
...
TEST_CASE("SyncSession: pause()/resume() API", "[sync][local][session]") {
    TestSyncManager init_sync_manager;
    auto app = init_sync_manager.app();
    auto user = app->sync_manager()->get_user("close-api-tests-user", ENCODE_FAKE_JWT("fake_refresh_token"),
                                              ENCODE_FAKE_JWT("fake_access_token"), "https://realm.example.org",
...
TEST_CASE("SyncSession: shutdown_and_wait() API", "[sync][local][session]") {
    TestSyncManager init_sync_manager;
    auto app = init_sync_manager.app();
    auto user = app->sync_manager()->get_user("close-api-tests-user", ENCODE_FAKE_JWT("fake_refresh_token"),
                                              ENCODE_FAKE_JWT("fake_access_token"), "https://realm.example.org",
...
TEST_CASE("SyncSession: update_configuration()", "[sync][local][session]") {
...
TEST_CASE("sync: error handling", "[sync][local][session]") {
...
TEMPLATE_TEST_CASE("sync: stop policy behavior", "[sync][local][session]", RegularUser)
{
    using ProtocolError = realm::sync::ProtocolError;
...
TEST_CASE("session restart", "[sync][local][session]") {
    if (!EventLoop::has_implementation())
        return;
...
TEST_CASE("sync: non-synced metadata table doesn't result in non-additive schema changes", "[sync][local][session]") {
    if (!EventLoop::has_implementation())
        return;
