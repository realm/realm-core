...
TEST_CASE("app: auth providers function integration", "[sync][app]")
{
    ...
    SECTION("auth providers function integration")
    {
        ...
        auto credentials =
            realm::app::AppCredentials::function(nlohmann::json({{"realmCustomAuthFuncUserId", "123456"}}).dump());
...
TEST_CASE("app: remote mongo client", "[sync][app]")
{
    ...
    app.provider_client<App::UsernamePasswordProviderClient>().register_email(
        email, password, [&](Optional<app::AppError> error) { CHECK(!error); });
    ...
    SECTION("insert")
    {
        ...
        collection.insert_one(dog_document, [&](Optional<std::string> document_json, Optional<app::AppError> error) {
            CHECK(!error);
        ...
        collection.insert_many(documents, [&](std::vector<std::string> inserted_docs, Optional<app::AppError> error) {
            CHECK(!error);
    ...
    SECTION("find")
    {
        ...
        collection.find(dog_document, [&](Optional<std::string> document_json, Optional<app::AppError> error) {
            CHECK(!error);
            auto json = nlohmann::json::parse(*document_json);
            CHECK(json.is_array());
        });

        collection.find_one(dog_document, [&](Optional<std::string> document_json, Optional<app::AppError> error) {
            CHECK(!error);
            CHECK(!document_json);
        });
        ...
        collection.find_one_and_delete(dog_document, find_and_modify_options,
                                       [&](Optional<app::AppError> error) { CHECK(!error); });
    ...
    SECTION("count and aggregate")
    {
        ...
        std::vector<std::string> pipeline = {"{\"$match\": { \"name\": \"fido\"}}",
                                             "{\"$group\": { \"_id\": \"$name\"}}"};
...
TEST_CASE("app: user_semantics", "[app]")
{
    ...
    const std::function<std::shared_ptr<SyncUser>(app::AppCredentials)> login_user = [&app](
        ...
        app.log_in_with_credentials(creds, [&](std::shared_ptr<realm::SyncUser> user, Optional<app::AppError> error) {
            CHECK(!error);
            test_user = user;
        });
        return test_user;
