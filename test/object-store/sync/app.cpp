...
TEST_CASE("app: user interrupts integration of remote changes", "[sync][app]") {
    std::string base_url = get_base_url();
    const auto partition = random_string(100);
    REQUIRE(!base_url.empty());
    ...
    auto server_app_config = minimal_app_config(base_url, "interrupt_integration", schema1);
    TestAppSession test_session(create_app(server_app_config));
    auto app = test_session.app();

    // Upload changes from first client.
    {
        SyncTestFile config(app, partition, schema1);
        auto realm = Realm::get_shared_realm(config);
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            auto obj = Object::create(c, realm, "TopLevel",
                                      std::any(AnyDict{{"_id", ObjectId::gen()}, {"list_of_strings", AnyVector{}}}));
    ...
    // Start second client and download the changes from first client.

    // Use different table name to avoid schema conflicts when merging changes from the other client.
    Schema schema2{
        {"TopLevel2",
         {
             {"_id", PropertyType::ObjectId | PropertyType::Nullable, true},
    ...
    SyncTestFile config(app, partition, schema2);

    SharedRealm realm;
    std::thread th;
    sync::version_type user_commit_version = UINT_FAST64_MAX;

    config.sync_config->on_download_message_received_hook = [&](std::weak_ptr<SyncSession>, const sync::SyncProgress&,
                                                                int64_t, sync::DownloadBatchState batch_state,
        ...
        auto func = [&] {
            ...
            TableRef table = wt.get_table("class_TopLevel2");
    ...
    CHECK(realm->read_group().get_table("class_TopLevel")->size() == 10); // objects downloaded
    CHECK(realm->read_group().get_table("class_TopLevel2")->size() == 1); // local user write
...
TEST_CASE("app: response error handling", "[sync][app]") {
    ...
    SECTION("session error code") {
        response.headers = util::HTTPHeaders{{"Content-Type", "application/json"}};
