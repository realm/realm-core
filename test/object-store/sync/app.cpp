...
TEST_CASE("app: response error handling", "[sync][app]") {
    ...
    SECTION("http 404") {
        ...
        CHECK(error.reason() == std::string("http error code considered fatal. Client Error: 404"));
        CHECK(error.link_to_server_logs.empty());
    }
    SECTION("http 500") {
        response.http_status_code = 500;
        ...
        CHECK(error.reason() == std::string("http error code considered fatal. Server Error: 500"));
        CHECK(error.link_to_server_logs.empty());
    }
