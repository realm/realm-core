                                       ...
                                       const std::string& partition, util::Optional<std::string> name)
{
    ...
    std::string manager_path = fs::path(util::format("%1mongodb-realm/%2/", base_path, cleaned_app_id));
    std::string preferred_name = fs::path(util::format("%1%2/%3", manager_path, identity, clean_name));
    current_preferred_path = fs::path(util::format("%1.realm", preferred_name));
    fallback_hashed_path = fs::path(util::format("%1%2.realm", manager_path, do_hash(preferred_name)));

    legacy_sync_directories_to_make.push_back(fs::path(util::format("%1%2", manager_path, local_identity)));
