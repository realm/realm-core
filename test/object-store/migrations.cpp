    ...
    SECTION("migration errors") {
        ...
        SECTION("change table to embedded - one incoming link per object") {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                long value = any_cast<long>(child_object.get_property_value<util::Any>(context, "value"));
                REQUIRE(value == 42 + i);
            }
        }
        ...
        SECTION("change table to embedded - multiple incoming links - resolved in migration block") {
            ...
            REQUIRE_NOTHROW(
                realm->update_schema(set_embedded(schema, "child_table", true), 2, [](auto, auto new_realm, auto&) {
                    ...
                    long value = any_cast<long>(child_object1.get_property_value<util::Any>(context, "value"));
