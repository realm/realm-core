...
TEST(LangBindHelper_AdvanceReadTransact_Basics)
{
    ...
    SharedGroup sg(hist, {Durability::durability_Full, crypt_key()});
...
TEST(LangBindHelper_AdvanceReadTransact_IntIndex)
{
    ...
    SharedGroup sg_r(*hist_r, {Durability::durability_Full, crypt_key()});
...
TEST(LangBindHelper_ImplicitTransactions)
{
    ...
    SharedGroup sg2(*hist2, {Durability::durability_Full, crypt_key()});
    Group& g = const_cast<Group&>(sg.begin_read());
...
TEST(LangBindHelper_ImplicitTransactions_OverSharedGroupDestruction)
{
    ...
    {
        SharedGroup sg(*hist1, {Durability::durability_Full, crypt_key()});
        {
            WriteTransaction wt(sg);
    ...
    {
        std::unique_ptr<Replication> hist2(make_client_history(path, crypt_key()));
        SharedGroup sg(*hist2, {Durability::durability_Full, crypt_key()});
        {
            WriteTransaction wt(sg);
...
TEST(LangBindHelper_MemOnly)
{
    ...
    {
        ShortCircuitHistory hist(path);
        SharedGroup sg(hist, {Durability::durability_MemOnly});
        WriteTransaction wt(sg);
        wt.add_table("table");
        wt.commit();
    }
    ...
    ShortCircuitHistory hist(path);
    SharedGroup sg_r(hist, {Durability::durability_MemOnly});
    SharedGroup sg_w(hist, {Durability::durability_MemOnly});
...
TEST(LangBindHelper_HandoverQuery)
{
    ...
    }
    {
        ...
        {
            sg_w.open(*hist_w, {Durability::durability_Full, crypt_key()});
            sg_w.begin_read();
            LangBindHelper::promote_to_write(sg_w);
...
TEST(LangBindHelper_LinkListCrash)
{
    ...
    std::unique_ptr<Replication> hist2(make_client_history(path, crypt_key()));
    SharedGroup sg2(*hist, {Durability::durability_Full, crypt_key()});
...
TEST(LangBindHelper_CompactLargeEncryptedFile)
{
...

    {
        ...
        SharedGroup sg(*hist, {Durability::durability_Full, crypt_key(true)});
        WriteTransaction wt(sg);
...
TEST(LangBindHelper_SessionHistoryConsistency)
{
    ...
    // session participants must still agree
    {
        ...
        std::unique_ptr<Replication> hist = realm::make_client_history(path, crypt_key());
        CHECK_LOGIC_ERROR(SharedGroup(*hist, {Durability::durability_Full, crypt_key()}),
...
TEST(LangBindHelper_InRealmHistory_Downgrade)
{
    ...
    }
    {
        // Out-of-Realm history
        std::unique_ptr<Replication> hist = make_client_history(path, crypt_key());
        CHECK_THROW(SharedGroup(*hist, {Durability::durability_Full, crypt_key()}),
                    InvalidDatabase);
    }
}
...
TEST(LangBindHelper_InRealmHistory_SessionConsistency)
{
    ...
    // The Realm is still at its initial empty state
    {
        ...
        CHECK_LOGIC_ERROR(SharedGroup(*hist_2, {Durability::durability_Full, crypt_key()}),
                          LogicError::mixed_history_type);
    }
}
