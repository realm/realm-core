...
TEST(LangBindHelper_AdvanceReadTransact_AddTableWithFreshSharedGroup)
{
    ...
    // Add the first table
    {
        std::unique_ptr<tightdb::Replication> repl_w(tightdb::makeWriteLogCollector(path));
    ...
    std::unique_ptr<tightdb::Replication> repl(tightdb::makeWriteLogCollector(path));
    SharedGroup sg(*repl);
    ReadTransaction rt(sg);
