...
void writer(std::string path, int id)
{
    ...
    try {
        bool done = false;
        SharedGroup sg(path, true, {Durability::durability_Full, crypt_key()});
...
void killer(TestContext& test_context, int pid, std::string path, int id)
{
    ...
    CHECK_EQUAL(0, child_exit_status);
    {
        ...
        SharedGroup sg(path, true, Durability::durability_Full);
        ReadTransaction rt(sg);
        rt.get_group().verify();
        TestTableShared::ConstRef t1 = rt.get_table<TestTableShared>("test");
...
TEST_IF(Shared_PipelinedWritesWithKills, false)
{
    ...
    {
        SharedGroup sg(path, false, {Durability::durability_Full, crypt_key()});
...
TEST(Shared_CompactingOnTheFly)
{
    ...
    writer_thread.join();
    {
        SharedGroup sg2(path, true, {Durability::durability_Full, crypt_key()});
        {
            sg2.begin_write();
            sg2.commit();
        }
...
TEST(Shared_InitialMem)
{
    SHARED_GROUP_TEST_PATH(path);
    {
        ...
        SharedGroup sg(path, no_create, {Durability::durability_MemOnly});
...
TEST(Shared_InitialMem_StaleFile)
{
    ...
    {
        bool no_create = false;
        SharedGroup(path, no_create, {Durability::durability_MemOnly});
    }
    CHECK(!File::exists(path));
...
TEST(Shared_Initial2)
{
    SHARED_GROUP_TEST_PATH(path);
    {
...

        {
            ...
            SharedGroup sg2(path, false, {Durability::durability_Full, crypt_key()});
...
TEST(Shared_Initial2_Mem)
{
    SHARED_GROUP_TEST_PATH(path);
    {
        ...
        {
            // Open the same db again (in empty state)
            SharedGroup sg2(path, no_create, {Durability::durability_MemOnly});
...
TEST(Shared_ManyReaders)
{
    ...
    for (int round = 0; round < num_rounds; ++round) {
        ...
        bool no_create = false;
        SharedGroup root_sg(path, no_create, {Durability::durability_MemOnly});
        ...
        for (int i = 0; i < 8*N; ++i)
            shared_groups[i].reset(new SharedGroup(path, no_create, {Durability::durability_MemOnly}));
...
{
    SHARED_GROUP_TEST_PATH(path);
    SharedGroup db(path, false, {Durability::durability_Full, crypt_key()});
...
TEST(Shared_MixedWithNonShared)
{
    ...
    {
        {
            SharedGroup sg(path, false, {Durability::durability_Full, crypt_key()}); // Create the very empty group
        }
...
TEST(Shared_SessionDurabilityConsistency)
{
    ...
    {
        bool no_create = false;
        Durability::Level durability_1 = Durability::durability_Full;
        SharedGroup sg(path, no_create, {durability_1});

        Durability::Level durability_2 = Durability::durability_MemOnly;
...
NONCONCURRENT_TEST(Shared_TopSizeNotEqualNine)
{
    ...
    SharedGroup sg3(path, false, {Durability::durability_Full, crypt_key()});
...
{
    SHARED_GROUP_TEST_PATH(path);
    SharedGroup sg_w(path, false, {Durability::durability_Full, crypt_key()});
