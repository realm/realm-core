...
// FIXME: This test was disabled because it has a strong tendency to leave rogue
// child processes behind after the root test process aborts. If these
// orphanned child processes are not manually searched for and killed, they will
// run indefinitely. Additionally, these child processes will typically grow a
// Realm file to gigantic sizes over time (100 gigabytes per 20 minutes).
//
// Idea for solution: Install a custom signal handler for SIGABRT and friends,
// and kill all spawned child processes from it. See `man abort`.
