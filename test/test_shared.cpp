...
{
    SHARED_GROUP_TEST_PATH(path);
    SharedGroup sg(path, false, SharedGroupOptions(0));
    std::string blob(16000000, 'x');
    ...
    for (;;) {
        ...
        for (size_t r = 0; r < 100; r++) {
            table->add_empty_row();
            table->set_binary_big(0, r, BinaryData(blob.data(), blob.size()));
        }
        wt.commit();
    }
}
...
ONLY(Shared_StaticFuzzTestRunSanityCheck)
{
    ...
    if (filename != "") {
        ...
        const size_t instructions = 20000;
