...
TEST(StringIndex_FullText)
{
    ...
    col.add("An object database (also object-oriented database management system) is a database management system in "
    ...
    col.add("Object database management systems grew out of research during the early to mid-1970s into having "
    ...
    const StringIndex& ndx = *col.create_fulltext_index();

    std::vector<ObjKey> result;

    ndx.find_all_fulltext(result, "object");
    ...
    // Insert with index
    col.insert(
        2, "As the usage of web-based technology increases with the implementation of Intranets and extranets, "
    ...
    result.clear();
    ndx.find_all_fulltext(result, "hybrid");
    CHECK_EQUAL(0, result.size());

    result.clear();
    ndx.find_all_fulltext(result, "gemstone");
    CHECK_EQUAL(1, result.size());

    col.erase(1);

    result.clear();
    ndx.find_all_fulltext(result, "æbler");
    CHECK_EQUAL(0, result.size());

    result.clear();
    ndx.find_all_fulltext(result, "texas");
    CHECK_EQUAL(1, result.size());
    ...
    col.add(
        ...
        "VOSS (Logic Arts) and JADE (Jade Software Corporation). Some of these products remain on the market and "
        "have been joined by new open source and commercial products such as InterSystems Caché.");
    col.add("As the usage of web-based technology increases with the implementation of Intranets and extranets, "
            "companies have a vested interest in OODBMSs to display their complex data. Using a DBMS that has been "
            "specifically designed to store data as objects gives an advantage to those companies that are geared "
            "towards multimedia presentation or organizations that utilize computer-aided design (CAD).[3]");
    col.add(
    ...
    result.clear();
    ndx.find_all_fulltext(result, "object gemstone");
    CHECK_EQUAL(2, result.size());

    // search for combination that is not present
    result.clear();
    ndx.find_all_fulltext(result, "object data");
    CHECK_EQUAL(0, result.size());

    // many terms
    result.clear();
    ndx.find_all_fulltext(result, "object database management brown");
    CHECK_EQUAL(1, result.size());
