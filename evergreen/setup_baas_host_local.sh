...
usage()
{
    echo "Usage: setup_baas_host_local.sh -f PATH -i PATH [-u USER] [-d PATH] -h"
...
while getopts "f:i:u:d:vh" opt; do
    case "${opt}" in
...
ssh "${SSH_OPTIONS[@]}" -L 9090:127.0.0.1:9090 "${SSH_USER}" "${FILE_DEST_DIR}/setup_baas_host.sh" -v -a "${FILE_DEST_DIR}/baas_host_vars.sh" -b master
