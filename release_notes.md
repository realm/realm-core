...
* Views can now be be kept synchronized with changes to the tables used to generate the view, use sync_if_needed() to do so. Views are no longer detached when the table they have been generated from are changed. Instead they just go out of sync. See further description in src/tightdb/table_view.hpp.
