...
// @@EndFold@@
    ...
    PeopleTable::View view2 = table.where().name.equal("Frank").find_all(table);
// @@Fold@@
    assert(view2.size() == 1 && view2[0].name == "Frank");
// @@EndFold@@
    // Find rows where male == true
    PeopleTable::View view3 = table.where().male.equal(true).find_all(table);
// @@Fold@@
    assert(view3.size() == 2 && view3[0].name == "Frank" && view3[1].name == "Bob");
// @@EndFold@@

    // Find people hired 2012-Jan-24, 00:00:00
    PeopleTable::View view4 = table.where().hired.equal(tightdb::Date(2012, 1, 24).get_date()).find_all(table); 
// @@Fold@@
