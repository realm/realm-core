...
identifier. In that case the client waits for an [IDENT](#ident) message from
...
can send its [IDENT](#ident-client-flx) message immediately after the [BIND](#bind) message.
In both cases, the client can send [UPLOAD](#upload) messages immediately after
it has sent the [IDENT](#ident-client-flx) message, and the server can send
...
ident>` to `0` (false), and then proceed to send an [IDENT](#ident-client-flx) message to
...
to the client in an [IDENT](#ident) message. When the client receives the
[IDENT](#ident) from the server, it must store the new file identifier
persistently in the client side Realm file, and then proceed to send an
[IDENT](#ident-client-flx) message to the server. Next time the client wants to initiate
a synchronization session for this file, it can send the [IDENT](#ident-client-flx) message
...
subserver in a star topology cluster. This is no longer supported and will always be zero. 
...
### [IDENT (Partition-based sync)][ident-client-pbs]
...
is not allowed to send the IDENT message until it has received an [IDENT](#ident)
message from the server for that session. Clients are not allowed to send more than
one IDENT message per session.

### [IDENT (Flexible sync)][ident-client-flx]

    head  =  'ident'  <session ident>
             <client file ident>  <client file ident salt>
             <download server version>  <download client version>
             <latest server version>  <latest server version salt>
             <query version> <query body size>

    body  =  <json-encoded FLX sync query>
...
Param: `<query version>` is the query version that the session will start with.
This must be the same query version as the active query the last time the file
ident was synchronizing with the server, or zero if this is a new file ident. The
query version may only increase over time.

Param: `<query size>` is the size in bytes of the JSON-encoded payload of the
FLX sync query. It must be the same as the queries sent in the active version
the last time the file ident was synchronizing with the server. If the
`query_version` is zero then this must be `2` and the body must be `{}`.
...
When the client requests a new file identifier via the [BIND](#bind) message, it
is not allowed to send the IDENT message until it has received an [IDENT](#ident-client-flx)
message from the server for that session. Clients are not allowed to send more than
one IDENT message per session.
...
[IDENT](#ident-client-flx) message for that session.
...
### QUERY
    head  =  'query' <session ident> <query version> <body size>

    body  =  <json encoded FLX sync queries>

Param: `<query version>` is the query version that this query message will change
the active query to. It is an int64_t encoded as a decimal. It should only increase
across the lifetime of the file and should not skip versions.

Param: `<body size>` is the size in bytes of the JSON-encoded body.

The body is a JSON object that represents the active queries on each table to been
synchronized. Each table should be a key in the JSON object and the value should be
a string representation of a valid RQL query. If a table exists but should not be
synchronized, it should be omitted from the object.

It is valid to send an empty JSON object to reflect that nothing should be synchronized.

The client must send QUERY messages for each FLX subscription set that exists and must
send them in the order they were created, sequenced with UPLOAD messages for writes that
occurred between changes of query.
...
synchronization sessions (see the client -> server [IDENT](#ident-client-flx) message).



### DOWNLOAD (Partition-based sync)
...
[IDENT](#ident-client-pbs) message). The latest changeset in the server's history is not
...
future synchronization sessions (see the client -> server [IDENT](#ident-client-pbs)
...
file ident>` as sent by the connected client in the [IDENT](#ident-client-pbs) message),
...
### DOWNLOAD (Flexible sync)

    head  =  'download'  <session ident>
             <download server version>  <download client version>
             <latest server version>  <latest server version salt>
             <upload client version>  <upload server version>
             <query version> <last in batch>
             <downloadable bytes>  <is body compressed>
             <uncompressed body size>  <compressed body size>
...
generated. The client must persist this value, and present it to the server when
initiating future synchronization sessions (see the client -> server
[IDENT](#ident-client-flx) message). The latest changeset in the server's history is not
necessarily sent in this DOWNLOAD message. The client can use `<latest server
version>` to estimate progress of the download process.
...
`<latest server version>`. The client must persist this value along with the
corresponding server version, and present it to the server when initiating
future synchronization sessions (see the client -> server [IDENT](#ident-client-flx)
message). The value is a nonzero positive integer strictly less than 2^63.
...
Param: `<query version>` is the query version that was active when this download
message was sent. This value will increase across a session, and the server is
allowed to skip versions. If this value has changed since the last DOWNLOAD
message received, or since the IDENT message was sent from the client, then the
server is beginning to bootstrap a new state in response to a [QUERY](#query)
message.

Param: `<last in batch>` is a boolean represented as a `1` or `0` that indicates
whether this DOWNLOAD message is part of a multi-part bootstrap. With this field
and the `<query version>` field, the client can tell whether the server is sending
it changesets generated from state or from tailing history.
...
which the original (untransformed) changeset was produced. This will never be
the file identifier of the connected client (i.e., it will never be `<client
file ident>` as sent by the connected client in the [IDENT](#ident-client-flx) message),
nor will it ever be any of the file identifiers allocated by the connected
client on behalf of its subordinate clients via [ALLOC](#alloc) messages.
The value is always a nonzero positive integer strictly less than 2^63.
...
The body of the message will be in JSON format with the following keys currently supported:
    ...
    - `rejectedUpdates` is a list of objects containing information about which objects
      caused a compensating write and why. Each object has a `reason`, `table`, and `pk` field
      representing the cause of the compensating write, the table name (without the `class_`
      prefix), and the primary key of the object that was reverted respectively.
...
| 226  | Query sent for FLX sync was invalid (IDENT, QUERY)
| 227  | Client tried to create an object that already exists outside their view (UPLOAD)
| 228  | Permissions have changed on the server since the last time this realm connected (IDENT)
| 229  | Initial sync is in progress - connect again later
| 230  | Client performed a write that is not allowed and a client reset is required (UPLOAD)
| 231  | Client performed a write that is not allowed and a compensating write was performed by the server (UPLOAD)
